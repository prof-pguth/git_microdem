...
uses
  ...
  ComCtrls, StdCtrls, ExtCtrls, Tabs, xif,
  petmar,Petmar_types;
...
type
  ...
  private
    ...
    procedure AddValue(ListView: TListView; const Name, Value: string; AddToTop: Boolean = False);
    procedure AddLoadErrorsValue(ListView: TListView; Section: TExifSection; AddToTop: Boolean = False);
  ...
  public
    ...
    procedure GetLatLong(var Lat,Long : ANSIString);
    function GetOrientation(var OrientationStr : ANSIString) : boolean;
    function GetViewDirection(var Dir: float): boolean;
  end;

implementation

uses
   ClipBrd, DateUtils, StrUtils, Themes;
...
begin
  case Value of
    srMissingOrInvalid, srKilometresPerHour: Result := 'km/h'; //km/h is the default according to the Exif spec
...
procedure TOutputFrame.GetLatLong(var Lat,Long : ANSIString);
var
  Item : TListItem;
  TStr : ANSIString;
begin
   {$IfDef RecordEXIF}
   WriteLineToDebugFile('TOutputFrame.GetLatLong in',true);
   {$EndIf}
  try
    for Item in ActiveListView.Items do begin
       {$IfDef RecordEXIF}
       WriteLineToDebugFile('"' + trim(Item.Caption) + '"    subitem count=' + IntToStr(Item.SubItems.Count));
       {$EndIf}

        if (Item.SubItems.Count <> 0) then begin
           TStr := trim(Item.Caption);
           if TStr = 'GPS latitude' then begin
              Lat := Item.SubItems[0];
           end;
           if TStr = 'GPS longitude' then begin
              Long := Item.SubItems[0];
           end;
        end;
    end;
  finally
...
function TOutputFrame.GetViewDirection(var Dir : float) : boolean;
var
  Item: TListItem;
  str : shortString;
begin
  try
    ...
    for Item in ActiveListView.Items do begin
        if Item.SubItems.Count <> 0 then begin
           if trim(Item.Caption) = 'GPS image direction' then begin
              Str := Item.SubItems[0];
              Result := Petmar.GetRealFromString(Str,Dir);
           end;
        end;
    end;
  finally
...
function TOutputFrame.GetOrientation(var OrientationStr : ANSIString) : boolean;
var
  Item: TListItem;
begin
   Result := false;
  try
    for Item in ActiveListView.Items do begin
        if Item.SubItems.Count <> 0 then begin
           if trim(Item.Caption) = 'Orientation' then begin
              OrientationStr := Item.SubItems[0];
              Result := true;
           end;
        end;
    end;
...
procedure TOutputFrame.TabSetChange(Sender: TObject; NewTab: Integer;  var AllowChange: Boolean);
var
...
var
  NewItem: TListItem;
begin
  if AddToTop then NewItem := ListView.Items.Insert(0)
  else NewItem := ListView.Items.Add;
...
begin
  ...
  try
    ...
    if ExifData.Empty then lsvStandard.Items.Add.Caption := 'No Exif metadata found'
    else LoadStandardValues(ExifData);
    if ExifData.HasMakerNote then LoadMakerNoteValues(ExifData.MakerNote, MakerNoteValueMap)
    else lsvMakerNote.Items.Add.Caption := 'No MakerNote found';
  finally
    lsvStandard.Items.EndUpdate;
    lsvMakerNote.Items.EndUpdate;
    ExifData.Free;
  end;
  if (imgThumbnail.Picture.Graphic <> nil) then begin
    grpThumbnail.Width := (grpThumbnail.Width - imgThumbnail.Width) + imgThumbnail.Picture.Width;
...
procedure TOutputFrame.LoadStandardValues(ExifData: TExifData);
  ...
  begin
    if Value <> 0 then AddValue(Name, IntToStr(Value));
  end;
  ...
  begin
    if DateTime <> 0 then AddValue(Name, DateTimeToStr(DateTime));
  end;

  procedure AddValue(const Name: string; const Fraction: TExifFraction; const Units: string = ''); overload;
  begin
    if not Fraction.MissingOrInvalid then AddValue(Name, '%g %s', [Fraction.Quotient, Units]);
  end;

  procedure AddValue(const Name: string; const Fraction: TExifSignedFraction); overload;
  begin
    if not Fraction.MissingOrInvalid then AddValue(Name, '%g', [Fraction.Quotient]);
  end;

  procedure AddValue(const Name: string; const Value: TSmallPoint); overload;
  begin
    if not InvalidPoint(Value) then AddValue(Name, '(%d, %d)', [Value.x, Value.y]);
  end;
  ...
  begin
    if not Direction.MissingOrInvalid then AddValue(Name, '%g %s', [Direction.Quotient, DirectionRefToStr(Ref)]);
  end;

  procedure AddValue(const Name: string; const Distance: TExifFraction; Ref: TGPSDistanceRef); overload;
  begin
    if not Distance.MissingOrInvalid then AddValue(Name, '%g %s', [Distance.Quotient, DistanceRefToStr(Ref)]);
  end;

  procedure AddValue(const Name: string; Resolution: TCustomExifResolution); overload;
  begin
    if not Resolution.MissingOrInvalid then
      AddValue(Name, '%g x %g %s', [Resolution.X.Quotient, Resolution.Y.Quotient,ResolutionUnitsToStr(Resolution.Units)]);
  end;
...
begin
  ...
  if DoSection(esGeneral, 'Main IFD') then begin
  ...
  if DoSection(esDetails, 'Exif sub-IFD') then begin
    ...
    if not ExifData.Flash.MissingOrInvalid then  begin
    ...
    if not ExifData.ISOSpeedRatings.MissingOrInvalid then AddValue('ISO speed rating(s)', ExifData.ISOSpeedRatings.AsString);
    ...
    if ExifData.ShutterSpeedInMSecs <> 0 then AddValue('Shutter speed', '%.4g milliseconds', [ExifData.ShutterSpeedInMSecs]);
  ...
  if DoSection(esInterop, 'Interoperability sub-IFD') then  begin
  ...
  if DoSection(esGPS, 'GPS sub-IFD') then begin
    ...
    AddValue('GPS altitude', ExifData.GPSAltitude, 'metres ' + GPSAltitudeRefToStr(ExifData.GPSAltitudeRef));
    ...
    AddValue('GPS image direction', ExifData.GPSImgDirection, ExifData.GPSImgDirectionRef);
    ...
    AddValue('GPS destination bearing', ExifData.GPSDestBearing,ExifData.GPSDestBearingRef);
    AddValue('GPS destination distance', ExifData.GPSDestDistance,ExifData.GPSDestDistanceRef);
    AddValue('GPS differential', GPSDifferentialToStr(ExifData.GPSDifferential));
  end;
  if DoSection(esThumbnail, 'Thumbnail IFD') then begin
...
procedure TOutputFrame.LoadMakerNoteValues(MakerNote: TExifMakerNote; ValueMap: TCustomIniFile);
...
var
  ...
  Tag: TExifTag;
begin
  if MakerNote is TUnrecognizedMakerNote then  begin
  ...
  begin
    ...
    if Tag.WellFormed and ValueMap.ReadBool(Section, 'TreatAsTagGroup', False) then
      for I := 0 to Tag.ElementCount - 1 do  begin
...
finalization
  {$IfDef RecordEXIF}
  WriteLineToDebugFile('RecordEXIF active in xifListFrame');
  {$EndIf}
end.
