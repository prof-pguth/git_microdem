...
uses
  ...
  Graphics,JPEG,
  {$Endif}
  Types, SysUtils, Classes,Contnrs, TypInfo,  xif.IPTC,
  xif.JPEGUtils, xif.StreamHelper, xif.TagIDs, xif.XMPUtils;

const
  SmallEndian = xif.StreamHelper.SmallEndian;
  BigEndian = xif.StreamHelper.BigEndian;

  xwAlwaysUpdate = xif.XMPUtils.xwAlwaysUpdate;
  xwUpdateIfExists = xif.XMPUtils.xwUpdateIfExists;
  xwRemove = xif.XMPUtils.xwRemove;

type
  EInvalidJPEGHeader = xif.JPEGUtils.EInvalidJPEGHeader; //= class(EInvalidGraphic);
  ECCRExifException = class(Exception);
  EInvalidTiffData = class(ECCRExifException);

  TEndianness = xif.StreamHelper.TEndianness;
...
uses
  SysConst, RTLConsts, Math, DateUtils, StrUtils, xif.Consts;
...
begin
  if MissingOrInvalid then
    ...
    FmtStr(Result, '%d%s%d%d', [Major, FormatSettings.DecimalSeparator, Minor, Release]);
end;
