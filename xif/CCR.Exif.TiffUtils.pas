...
uses
  ...
  CCR.Exif.BaseUtils, CCR.Exif.StreamHelper;
...
uses
   ...
   Contnrs,
   {$EndIf}
   Math, RTLConsts, CCR.Exif.Consts, CCR.Exif.TagIDs;
...
constructor TTiffDirectoryRewriter.Create(const Source: ITiffDirectory; const Callback: ITiffRewriteCallback);
  {$IfDef Android}
  begin
  {$Else}
         function RewriteOldTag(TagID: TTiffTagID; DataType: TTiffDataType; ElementCount: LongInt; out Index: Integer): Boolean;
         begin
          Result := TTiffTag.IsWellFormed(DataType, ElementCount) and not FindTagIndex(TagID, Index);
          if Result and (Callback <> nil) then Callback.RewritingOldTag(Source, TagID, DataType, Result);
         end;
...
begin
  ...
  if Supports(Source, IFoundTiffDirectory, IntDirectory) then begin
    FSourceInfo := IntDirectory.TagInfo;
    FSourceParser := IntDirectory.Parser;
    TagPtr := PTiffTagInfo(FSourceInfo);
    for I := 0 to High(FSourceInfo) do begin
      if RewriteOldTag(TagPtr.ID, TagPtr.DataType, TagPtr.ElementCount, TagIndex) then
        if not FSourceParser.ParseSubDirectory(Source, TagPtr^, SubDir) then
          ...
          if SubRewriter.RewriteSource then FTagsToWrite.Insert(TagIndex, TTiffSubDirToWrite.Create(Self, TagPtr.ID, SubRewriter))
          else SubRewriter.Free;
        end;
      Inc(TagPtr);
    end;
  end
...
procedure WriteTiffHeader(Stream: TStream; Endianness: TEndianness; FirstDirectoryOffset: LongWord = 8);
{$IfDef Android}
begin
{$Else}
begin
  if Endianness = SmallEndian then begin
