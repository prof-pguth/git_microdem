...
uses
  ...
  Graphics,JPEG,
  {$Endif}
  Types, SysUtils, Classes, xif.StreamHelper;
...
const
  ...
  StartOfFrameMarkers  = [jmStartOfFrame0..jmStartOfFrame3, jmStartOfFrame5..jmStartOfFrame7]; //there's no jmStartOfFrame4
  MarkersWithNoData = [$00, $01, $D0..$D9];

function JPEGHeader(JPEGStream: TStream; const MarkersToLookFor: TJPEGMarkers;  StreamOwnership: TStreamOwnership = soReference): IJPEGHeaderParser; overload;
function JPEGHeader(const JPEGFile: string;  const MarkersToLookFor: TJPEGMarkers = AnyJPEGMarker): IJPEGHeaderParser; overload; inline;
function JPEGHeader(Image: TJPEGImage; const MarkersToLookFor: TJPEGMarkers = AnyJPEGMarker): IJPEGHeaderParser; overload;
...
procedure WriteJPEGSegmentToStream(Stream: TStream; MarkerNum: TJPEGMarker; const Data; DataSize: Word); overload;
procedure WriteJPEGSegmentToStream(Stream: TStream; MarkerNum: TJPEGMarker; Data: TStream; DataSize: Word = 0); overload;
...
function CreateAdobeBlock(ATypeID: Word; const AName: AnsiString;  const ADataSource: IStreamPersist = nil): IAdobeBlock; overload;
function CreateAdobeBlock(const ASignature: AnsiString; ATypeID: Word;  const AName: AnsiString; const ADataSource: IStreamPersist = nil): IAdobeBlock; overload;
...
type
  ...
  public
    ...
    constructor Create(const ASignature: AnsiString; ATypeID: Word; const AName: AnsiString; const ADataSource: IStreamPersist); overload;
    destructor Destroy; override;
  end;
