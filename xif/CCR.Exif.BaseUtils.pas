...
uses
  ...
  CCR.Exif.StreamHelper,CCR.Exif.Consts;
...
type
  ...
  protected type
    ...
    protected
      function Exif_Realloc(var NewCapacity: Longint): Pointer; //override;
...
type
  ...
  protected
    ...
    function MoveNext: Boolean;
  public
    constructor Create(JPEGStream: TStream; const MarkersToLookFor: TJPEGMarkers; StreamOwnership: TStreamOwnership);
    destructor Destroy; override;
  end;
...
begin
  {$IfDef Android}
  SetString(S, PChar(UTF8Buffer), ByteLen);
  {$Else}
...
function TMetadataBlock.TDataStream.Exif_Realloc(var NewCapacity: Longint): Pointer;
begin
  if DisableChanges then
    raise EStreamError.CreateRes(@SStreamIsReadOnly)
  else
    Result := {inherited} Exif_Realloc(NewCapacity);
end;
...
begin
  {$IfDef Android}
  Result := StringOfChar(Char(' '), 4);
  StrPLCopy(PChar(Result), S, 4);
  {$Else}
...
var
  ...
  DataSize: Word;
  {$IfDef Android}
  Buffer,MaxPos, SeekPtr : PChar;
  {$Else}
  Buffer,MaxPos, SeekPtr : PAnsiChar;
  {$EndIf}
begin
  FCurrent := nil;
  Result := False;
