...
var
   ...
   fName : PathStr;
   IndexTable : tMyData;
   NumCats,Category,Int,TilesInIndexTable : integer;
   ...
   sl,Cats,Tiles : tStringList;
         ...
         function ReadPointCloudPropertiesFromTable : boolean;
         var
            fName : PathStr;
         begin
            fName := IndexTableName;
            {$IfDef RecordLASHistfull} WriteLineToDebugFile('Read Existing ' + fName); {$EndIf}
            IndexTable := tMyData.Create(fName);
            Result := (IndexTable.RecordCount > 0);
            if Result then begin
               TilesInIndexTable := IndexTable.RecordCount;
            ...
            else TilesInIndexTable := 0;
            IndexTable.Destroy;
            {$IfDef RecordLASHistfull} WriteLineToDebugFile('Done Existing ' + fName); {$EndIf}
         end;
         ...
         procedure DoResults(fName : PathStr;  LasData : tLas_Data);
                 ...
                 begin
                    ...
                    Cats.Add('');
                    {$IfDef RecordLASHistfull} WriteLineToDebugFile('tLas_files.ElevationHistogram done scan angle'); {$EndIf}
                    ...
                    {$IfDef RecordLASHistfull} WriteLineToDebugFile('tLas_files.ElevationHistogram done GPS time'); {$EndIf}
                     ...
                     Cats.Add('');
                     {$IfDef RecordLASHistfull} WriteLineToDebugFile('tLas_files.ElevationHistogram done point cloud ID'); {$EndIf}
                 ...
                 begin
                    ...
                    {$IfDef RecordLASHistfull} WriteLineToDebugFile('tLas_files.ElevationHistogram done intensity'); {$EndIf}
                    if HasRGB then begin
                       FindMinMax('Red',Reds^);
                       ...
                       {$IfDef RecordLASHistfull} WriteLineToDebugFile('tLas_files.ElevationHistogram done rgb'); {$EndIf}
                    end
                 ...
                 First := theLowElev;
                    ...
                    if DoGraph then begin
                       ...
                       GraphFromCSVfile(sl,false,true);
                      {$IfDef RecordLASHistfull} WriteLineToDebugFile('tLas_files.ElevationHistogram done elevation graph'); {$EndIf}
                    end
                    else begin
                      {$IfDef RecordLASHistfull} WriteLineToDebugFile('tLas_files.ElevationHistogram skip elevation graph'); {$EndIf}
                    end;
                 end;
...
begin
   ...
   if (CloudStats = csLoadStats) and FileExists(IndexTableName) then begin
      if ReadPointCloudPropertiesFromTable then begin
         TStr := 'done read from table, tiles in index table = '+ IntToStr(TilesInIndexTable);
         if (LAS_fnames.Count = TilesInIndexTable) then begin
      ...
      for k := 0 to pred(LAS_fnames.Count) do begin
         ...
         if (IndexTable <> Nil) then begin
            ...
            {$IfDef RecordLASHistful} WriteLineToDebugFile('tLas_files.ElevationHistogram ready to free'); {$EndIf}
            LasData.FreeLASRecordMemory;
            {$IfDef RecordLASHistfull} WriteLineToDebugFile('tLas_files.CloudStatistics call destroy'); {$EndIf}
            LasData.Destroy;
         end;
      end;
...
var
   ...
   lf : tLAS_data;
begin
   {$IfDef Defined(RecordListFilesProcessed) or Defined(RemoveTiles)} WriteLineToDebugFile('tLas_files.RemoveTilesNotOnMap in'); {$EndIf}
   ...
   EndProgress;
   {$IfDef Defined(RecordListFilesProcessed) or Defined(RemoveTiles) or Defined(RecordHemisphere)}
      WriteLineToDebugFile('tLas_files.RemoveTilesNotOnMap out, UTM zone=' + IntToStr(UTMzone) + LatHemi);
   {$EndIf}
end;
