...
type
  Tpt_cloud_opts_fm = class(TForm)
    ...
    Label36: TLabel;
    BitBtn65: TBitBtn;
    ArcsecondDTMDSMforrangeofscales1: TMenuItem;
    Label37: TLabel;
    Label38: TLabel;
    Edit35: TEdit;
    Edit36: TEdit;
    ...
    procedure ArcsecondDTMDSMforrangeofscales1Click(Sender: TObject);
...
procedure Tpt_cloud_opts_fm.WBT_MakeDTM(Mode : integer);
var
   i : integer;
   LasName,DTMName : PathStr;
   CombinedEPSG : shortstring;
begin
    for i := 1 to MaxClouds do if UsePC[i] and (LasFiles[i] <> Nil) then begin
        CombinedEPSG := '';
        if (LasFiles[i].VersionMajor = #1) and (LasFiles[i].VersionMinor = #4) then begin
           CombinedEPSG := LasFiles[i].H_datumcode + '+' + LasFiles[i].V_datumcode;
        end;
        if Mode = 1 then WBT_IDWCreate(true,LasFiles[i].CloudDir,MDTempDir + LasFiles[i].CloudName + '_wbidw_dtm.tif',MDdef.DefLidarXGridSize,CombinedEPSG);
        if Mode = 2 then WBT_BNearNeighCreate(true,LasFiles[i].CloudDir,MDTempDir + LasFiles[i].CloudName + '_wbidw_dtm.tif',MDdef.DefLidarXGridSize,CombinedEPSG);
    end;
end;
...
procedure Tpt_cloud_opts_fm.ArcsecondDTMDSMforrangeofscales1Click(Sender: TObject);


   procedure DoSpacing(space: float64);
   var
      DTM,DSM : integer;
   begin
      {$IfDef RecordMakeGrid} WriteLineToDebugFile('Start, spacing=' + RealToString(space,-12,-2)); {$EndIf}
       MDdef.DefLidarGeoGridSizeY := space;
       MDDef.MakePCFloor := false;
       MDDef.MakePCCeiling := true;
       DSM := MakeGrid(pcgmCeilFloor);

//ADD RESOLUTION TO THE DEM NAME, AND CHECK IF IT SHOULD BE 0.05

       DEMGlb[DSM].SaveAsGeotiff(AutoSaveDir + DEMglb[DSM].AreaName + '.tif');
       DTM := MakeGrid(pcgmGrndPtDTM);
       DEMGlb[DTM].SaveAsGeotiff(AutoSaveDir + DEMglb[DTM].AreaName + '.tif');
   end;


begin
   {$IfDef RecordMakeGrid} WriteLineToDebugFile('Tpt_cloud_opts_fm.ArcsecondDTMDSMforrangeofscales1Click in'); {$EndIf}
   If (AutoSaveDir = '') then GetDosPath('saved grids',AutoSaveDir);
   MdDef.ForceSquarePixels := true;
   ...
   DoSpacing(0.5);
   DoSpacing(1);
   {$IfDef RecordMakeGrid} WriteLineToDebugFile('Tpt_cloud_opts_fm.ArcsecondDTMDSMforrangeofscales1Click out'); {$EndIf}
end;
...
begin
   ...
   if MDdef.AutoReDrawMapLAS then begin
      ...
      if MDDef.ShowCloudOutlines then MapCallOutlineClouds;
   end
   else begin
...
procedure Tpt_cloud_opts_fm.PageControl1Change(Sender: TObject);
begin
   if (PageControl1.ActivePage = TabSheet8) or (PageControl1.ActivePage = TabSheet6) then begin
      Edit35.Text := LasFiles[1].H_DatumCode;
      Edit36.Text := LasFiles[1].V_DatumCode;
      AutoSaveDir := LasFiles[1].CloudDir + 'lidar_grids\';
...
procedure Tpt_cloud_opts_fm.BitBtn65Click(Sender: TObject);
begin
...
var
   ...
   fName : PathStr;
      ...
      begin
         ...
         {$IfDef RecordMakeBaseMap} WriteLineToDebugFile('CreateNewUTMGrid ' + DEMGlb[Result].FullDEMParams); {$EndIf}
...
begin
   ...
   if AutoLoad and ValidPath(BaseDir) then begin
      ...
      if MDDef.PickLASDirs then begin
         ...
         if not Petmar.GetMultipleFiles('Lidar files','LAS files|*.LAS',LasFiles[CloudNum].LAS_fnames,DefFilter) then exit;
   ...
   //LasFiles[CloudNum].CloudBaseDir := BaseDir;

   for i := pred(LasFiles[CloudNum].LAS_fnames.Count) downto 0 do begin
   ...
   if Result then begin
      ...
      if (BaseMap = Nil) then begin
         ...
         DEMBase := CreateNewUTMGrid(LasFiles[CloudNum].CloudName + '_cloud',MakeBox,2);
...
var
   ...
   aTable : Petmar_db.tMyData;
begin
   {$IfDef BulkGrids}  WriteLineToDebugFile(''); WriteLineToDebugFile('Tpt_cloud_opts_fm.DTMrangescalesfromgroundpoints1Click start'); {$EndIf}
   ...
   if Petmar.GetExistingFileName('DTM creation parameters','*.dbf',fName) then begin
      ...
      while not aTable.eof do begin
         ...
         wmDEM.SetPanelText(0,NewName);
         {$IfDef BulkGrids} WriteLineToDebugFile('Start ' + fName); {$EndIf}
         ...
         if ValidDEM(NewDEM) then begin
            ...
            {$IfDef BulkGrids} WriteLineToDebugFile('Saved: ' + fName); {$EndIf}
         end;
         aTable.Next;
      end;
      aTable.Destroy;
   ...
   wmDEM.SetPanelText(0,'');
   {$IfDef BulkGrids} WriteLineToDebugFile('Tpt_cloud_opts_fm.DTMrangescalesfromgroundpoints1Click done'); {$EndIf}
end;
...
begin
   ...
   if MdDef.ForceSquarePixels then Edit25.Text := Edit21.Text;
end;
