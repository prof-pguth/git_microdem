...
type
  ...
  private
    FHexChar: AnsiChar;
    FDecimSeparator: AnsiChar; // default SysUtils.DecimalSeparator
    FArgSeparator: AnsiChar; // default SysUtils.ListSeparator
    FOptimize: Boolean;
    ConstantsList: TOCollection;
    LastRec: PExpressionRec;
    CurrentRec: PExpressionRec;
    function ParseString(AnExpression: ANSIstring): TExprCollection;
    ...
    function CompileExpression(AnExpression: ANSIstring): Boolean;
    ...
    procedure SetArgSeparator(const Value: ANSIchar);
    procedure SetDecimSeparator(const Value: ANSIchar);
  protected
  ...
  public
    ...
    procedure DefineVariable(AVarName: ANSIstring; AValue: PDouble);
    procedure DefineStringVariable(AVarName: ANSIstring; AValue: PString);
    procedure DefineFunction(AFunctName, ADescription: ANSIstring; AFuncAddress:
      TDoubleFunc; NArguments: Integer);
    procedure DefineStringFunction(AFunctName, ADescription: ANSIstring;
      AFuncAddress: TStringFunc);
    procedure ReplaceFunction(OldName: ANSIstring; AFunction: TObject);
    function Evaluate(AnExpression: ANSIstring): Double;
    function EvaluateCurrent: Double; //fastest
    function AddExpression(AnExpression: ANSIstring): Integer; virtual;
    ...
    property ArgSeparator: ANSIchar read FArgSeparator write SetArgSeparator;
    property DecimSeparator: ANSIchar read FDecimSeparator write SetDecimSeparator;
  ...
  private
    ...
    function GetAsHexadecimal(AIndex: Integer): ANSIstring;
    function GetExpression(AIndex: Integer): ANSIstring;
  protected
  ...
  public
    constructor Create;
    destructor Destroy; override;
    function AddExpression(AnExpression: ANSIstring): Integer;  override;
    procedure ClearExpressions; override;
    property ExpressionSize[AIndex: Integer]: Integer read GetExprSize;
    property Expression[AIndex: Integer]: ANSIstring read GetExpression;
    property AsFloat[AIndex: Integer]: Double read GetResults;
    property AsString[AIndex: Integer]: ANSIstring read GetAsString;
    property AsBoolean[AIndex: Integer]: Boolean read GetAsBoolean;
    property AsHexadecimal[AIndex: Integer]: ANSIstring read GetAsHexadecimal;
...
function _pos(s1,s2:ANSIstring):double;
begin
  Result:=pos(s1,s2);
end;
...
function TCustomExpressionParser.CompileExpression(AnExpression: ANSIstring):
  Boolean;
var
...
begin
//hard wired  3/18/2010 in going to Unicode, PLG
    ...
    repeat
      ...
      if IEnd = Expr.Count then
        raise EParserException.Create(errorprefix+'Syntax error in expression ' +
          CurrentExpression);
      if TExprWord(Expr.Items[IEnd]).NFunctionArg > 0 then begin
      ...
      if not FirstOper.IsOper then begin // parse function arguments
        IArg := 0;
        IStart := IEnd + 1;
        IEnd := IStart;
        if TExprWord(Expr.Items[IEnd]).VarType = vtLeftBracket then
          ...
          brCount := 0;
        while (IEnd < Expr.Count - 1) and (brCount <> 0) do  begin
          Inc(IEnd);
...
function TCustomExpressionParser.ParseString(AnExpression: ANSIstring):
  TExprCollection;
var
  isConstant: Boolean;
  I, I1, I2, Len: Integer;
  ...
  procedure ReadConstant(AnExpr: ANSIstring; isHex: Boolean);
  begin
    isConstant := True;
  ...
  procedure ReadWord(AnExpr: ANSIstring);
  var
    OldI2: Integer;
  begin
    isConstant := False;
      ...
      begin
        ...
        case AnExpr[I2] of
          ...
          '+':
            begin
              Inc(I2);
              if (I2 <= Len)and(AnExpr[I2] = '+') and WordsList.Search(pANSIchar('++'), I) then
                Inc(I2);
            end;
          '-':
            begin
...
//removed 3/18/2010 in going to Unicode, PLG
  ...
  repeat
    ...
    begin
      ...
      if WordsList.Search(pANSIchar(W), I) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if K = -1 then
        begin
          if WordsList.Search(pANSIchar('-@'), J) then
            Items[I] := WordsList.Items[J];
        end
        else if WordsList.Search(pANSIchar('+@'), J) then
          Items[I] := WordsList.Items[J];
      end;
      {----CHECK ON DOUBLE NOT----}
...
procedure TCustomExpressionParser.DefineVariable(AVarName: ANSIstring; AValue:
  PDouble);
begin
  AddReplaceExprWord(TDoubleVariable.Create(AVarName, AValue));
end;

procedure TCustomExpressionParser.DefineStringVariable(AVarName: ANSIstring; AValue:
  PString);
begin
...
function TCustomExpressionParser.Evaluate(AnExpression: ANSIstring): Double;
begin
...
function TCustomExpressionParser.AddExpression(AnExpression: ANSIstring): Integer;
begin
  if AnExpression <> '' then
  begin
    Result := 0;
...
procedure TCustomExpressionParser.ReplaceFunction(OldName: ANSIstring; AFunction:
  TObject);
var
  I: Integer;
begin
  if WordsList.Search(pANSIchar(OldName), I) then
  begin
...
var
  IOldVar: Integer;
begin
  if WordsList.Search(pANSIchar(AExprWord.Name), IOldVar) then
  begin
...
function TCustomExpressionParser.GetFunctionDescription(AFunction: ANSIstring):
  ANSIstring;
var
  S: ANSIstring;
  p, I: Integer;
begin
  ...
  if p > 0 then
    S := Copy(S, 1, p - 1);
  if WordsList.Search(pANSIchar(S), I) then
...
procedure TCustomExpressionParser.DefineStringFunction(AFunctName, ADescription: ANSIstring; AFuncAddress: TStringFunc);
begin
  AddReplaceExprWord(TSimpleStringFunction.Create(AFunctName, ADescription,  AFuncAddress,  nil, nil));
end;

procedure TCustomExpressionParser.SetArgSeparator(const Value: ANSIchar);
begin
...
procedure TCustomExpressionParser.SetDecimSeparator(const Value: ANSIchar);
begin
...
function TExpressionParser.AddExpression(AnExpression: ANSIstring): Integer;
begin
  if AnExpression <> '' then
  begin
...
function TExpressionParser.GetAsString(AIndex: Integer): ANSIstring;
var
  D: Double;
begin
  D := AsFloat[AIndex];
...
{$ENDIF}
    ...
    Add(TFunction.Create('radtodeg', 'conversion of rad to degrees', _RadToDeg,
      1));
    DefineStringFunction('pos','position in string',_pos)
    {pos is an example of a valid string function:  the expression: pos('a','bcad') evaluates as 3
      }
     end;
end;

function TExpressionParser.GetAsHexadecimal(AIndex: Integer): ANSIstring;
var
  D: Double;
begin
  D := AsFloat[AIndex];
...
function TExpressionParser.GetExpression(AIndex: Integer): ANSIstring;
begin
...
function TExpressionParser.CurrentExpression: ANSIstring;
begin
