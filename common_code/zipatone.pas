...
begin
   {$IfDef RecordZipatoneProblems} WriteLineToDebugFile('Pattern file read: ' + PatternFileName + '  in'); {$EndIf}
   if (StandardPattern = Nil) then  begin
      ...
      if not FileExists(PatternFileName) then begin
         {$IfDef RecordZipatoneProblems} WriteLineToDebugFile('Pattern file missing: ' + PatternFileName); {$EndIf}
         result := false;
         exit;
      end {while};
      ...
      close(PatternFile);
      Result := true;
      {$IfDef RecordZipatoneProblems}WriteLineToDebugFile('Pattern file read: ' + PatternFileName + ' out, Patterns=' + IntToStr(NumStandardPattern)); {$EndIf}
   end
   else begin
      {$IfDef RecordZipatoneProblems} WriteLineToDebugFile('Pattern file already read'); {$EndIf}
   end;
