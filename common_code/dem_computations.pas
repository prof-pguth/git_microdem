...
var
   ...
   fName : PathStr;
         ...
         var
            d0,d1,d2,Bearing,dx,dy : float64;
         begin
...
procedure HowBigIsAnArcSecond;
//the options to do for the southern hemisphere removed, since they are identical

         procedure DoForIncrement(StepIncrement : float64);
         var
            Long : float64;
            Results : tStringList;
            fName : PathStr;
               ...
               begin
                  ...
                  (*
                  Lat := -Lat;
                  VincentyCalculateDistanceBearing(Lat + 0.5 /3600,Long,Lat-0.5 /3600,Long,sdx,Bearing);
                  VincentyCalculateDistanceBearing(Lat,Long+0.5 /3600,Lat,Long - 0.5 /3600,sdy,Bearing);
                  *)
                  Results.Add(RealToString(Lat,-8,2) + ',' + RealToString(dx,12,4) + ',' + RealToString(dy,12,4) {+ ',' + RealToString(sdx,12,4) + ',' + RealToString(sdy,12,4)});
               end;
         ...
         begin
            Results := tStringList.Create;
            Results.Add('LAT,DX_M,DY_M' {,SDX,SDY'});
            long := 0;
            lat := 85;
            while lat >= 0 do begin
               DoPixelSize(Lat);
               Lat := Lat - StepIncrement;
            end;

            fName := MDTempDir + 'Arc_Second_Length_every_' + RealToString(StepIncrement,-8,-2) + '_degree.csv';
...
begin
   ...
   for i := 1 to NumMods do begin
      ThisGraph.MainSymbol.Color := ConvertTColorToPlatformColor(WinGraphColors(i));
      ThisGraph.OpenPointSymbolFile(rf[i],theNames[i],ThisGraph.MainSymbol);
   end;
   ca[1] := vcTM5441;
   ca[2] := vcYoeli;
