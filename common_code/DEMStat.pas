...
var
   ...
   fName : PathStr;
begin
   {$IfDef RecordHistogram} WriteLineToDebugFile('CreateGridHistograms in'); {$EndIf}
   ...
   for j := 1 to MaxDEMDataSets do begin
      if DEMsWanted[j] and ValidDEM(j) then begin
         ...
         ThisMin := Values^[round(NPts * (100 - TailCutoff) / 100)];
         ThisMax := Values^[round(NPts * TailCutoff / 100)];
...
var
   ...
   IdenticalGrids : boolean;
begin
   {$IfDef RecordGridScatterGram} WriteLineToDebugFile('GridScatterGram in, want visible=' + TrueOrFalse(not MDDef.CreateGraphHidden)); {$EndIf}
   ...
   if ValidDEM(DEM1) and ValidDEM(DEM2) then begin
      {$IfDef RecordGridScatterGramFull}
      ...
      NPts := 0;
      Prog := 0;
      {$IfDef RecordGridScatterGram} WriteLineToDebugFile('GridScatterGram start scatter, visible=' + TrueOrFalse(Result.Visible)); {$EndIf}
      ...
      {$IfDef RecordGridScatterGram} WriteLineToDebugFile('GridScatterGram in, points added=' + IntToStr(NPts)); {$EndIf}
      ...
      {$IfDef RecordGridScatterGram} WriteLineToDebugFile('GridScatterGram out, visible=' + TrueOrFalse(Result.Visible)); {$EndIf}
   end;
end;
