...
begin {proc CreateGraphAxes}
   ...
   with GraphDraw do begin
      ...
      if (GraphDraw.GraphLeftLabels <> Nil) and (GraphDraw.GraphLeftLabels.Count > 0) and GraphDraw.ShowGraphLeftLabels then begin
         if StrUtils.AnsiContainsText(GraphDraw.GraphLeftLabels.Strings[0],',') then begin
            for I := 1 to GraphDraw.GraphLeftLabels.Count do begin
               ...
               yf := StrToFloat(BeforeSpecifiedCharacterANSI(Tstr,',',true,true));
               Bitmap.Canvas.TextOut(5,GraphY(yf)-Bitmap.Canvas.TextHeight(TStr) div 2,RemoveUnderScores(TStr));
            end;
         end
         else begin
            for I := 1 to GraphDraw.GraphLeftLabels.Count do begin
      ...
      if FileExists(BottomLegendFName) and (GraphDraw.BottomLegendFName <> '') and (not GraphDraw.MultiPanelGraph) then begin
...
begin
   ...
   BottomMargin := MarginFreeboard + 5 * Bitmap.Canvas.TextHeight(HorizLabel);
...
begin
   ...
   for i := 1 to GraphDraw.DataFilesPlotted.Count do begin
       ...
       if (infName <> '') then Results.SaveToFile(infName)
          ...
          if (GraphDraw.LegendList <> Nil) then fName := NextFileNumber(MDTempDir,Caption + '_graph_' + GraphDraw.LegendList.Strings[pred(i)] + '_', '.dbf')
          else fName := NextFileNumber(MDTempDir,'Graph_series_', '.dbf');
          StringList2CSVtoDB(Results,fName);
       end;
   end;
...
var
   ...
   begin
      ...
      GraphDraw.UpperLeftText := {'r=' + RealToString(r,-8,3) +} 'r²=' + RealToString(sqr(r),-8,4);
      RedrawDiagram11Click(Nil);
   end;
...
function TThisBaseGraph.MakeLegend : tMyBitmap;
   ...
   function MakeTheLegend(Flist : tstringList; Extract : boolean) : tMyBitmap;
   const
      ItemHigh = 25;
   var
      ...
      TStr : shortString;
   begin
      if (Flist <> Nil) and (Flist.Count > 0) then begin
...
begin
  Result := nil;
  if (GraphDraw.DBFLineFilesPlotted <> Nil) and (GraphDraw.DBFLineFilesPlotted.Count > 0) then Result := MakeTheLegend(GraphDraw.DBFLineFilesPlotted,true)
  else if (GraphDraw.LegendList <> Nil) and (GraphDraw.LegendList.Count > 0) then Result := MakeTheLegend(GraphDraw.LegendList,false);
...
begin
   if (GraphDraw.DBFLineFilesPlotted.Count > 0) or (GraphDraw.LegendList <> Nil) then AssignBitmapToClipBoard(MakeLegend);
...
var
  ...
  x : integer;
begin
   if (GraphDraw.LegendList <> nil) and (GraphDraw.LegendList.Count > 0) then begin
