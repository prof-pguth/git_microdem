      ...
      if Single then begin
         ...
         if (View3D.FlightRouteDB <> Nil) then begin
            ...
            if FlightPathGraphsPossible and MDDef.ShowAlongTrackProfiles then begin
               ...
               AlongTrackGlide.OpenPointFile(rfile,AlongTrackGlide.Symbol);
               AlongTrackGlide.Symbol.Color := ConverttColorToPlatformColor(clGray);
               AlongTrackGlide.OpenPointFile(rfile2,AlongTrackGlide.Symbol);
               AlongTrackGlide.Symbol.Color := ConverttColorToPlatformColor(clSilver);
               AlongTrackGlide.OpenPointFile(rfile3,AlongTrackGlide.Symbol);
               View3D.FlightRouteDB.First;
...
begin
   if (View3D.MainInCh = 'P') and DEMPerOp.SetPerspectiveOptions(View3D,View3D.PersSize) then begin
      DrawPerspective(false);
   end;
end;
...
begin
   ...
   if (View3D.NumRadialPts > PetMath.MaxFArrayPts) then View3D.NumRadialPts := pred(PetMath.MaxFArrayPts);
...
begin
   with View3D,DEMGlb[DEMOnView],SelectionMap.MapDraw do if (MainInCh in ['P']) then begin
      ...
      if FindLatLong(Lat,Long,Pitch,DistOut,Azimuth,MessZ,Mess2,Mess3,Mess4,(not PersPitchAzimuthDigitize)) then {with Image1.Canvas do} begin
         ...
         (*
         MolodenskiyTransformation(Lat,Long,Lat,Long,PrimMapProj,SecondaryMapProjection);
         Mess1 := Mess1 + SecondaryMapProjection.h_DatumCode + '   ' + LatLongDegreeToString(Lat,Long,MDDef.OutPutLatLongMethod) +  MessLineBreak +
              SecondaryMapProjection.UTMStringFromLatLongDegree(Lat,Long) + MessLineBreak+ '  ' + MessLineBreak;
         *)
