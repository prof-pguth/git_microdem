...
type
  TMapForm = class(TForm)
    ...
    NoZoom1: TMenuItem;
    ZoomOut2: TMenuItem;
    //Interpolatesatellitecolors1: TMenuItem;
    ...
    //Pointslopealgorithm1: TMenuItem;
    Keyboard1: TMenuItem;
    N16: TMenuItem;
    N17: TMenuItem;
    N18: TMenuItem;
    ...
    //LabelTIGERroads1: TMenuItem;
    //LocateTIGERRoad1: TMenuItem;
    N29: TMenuItem;
    Hotspots1: TMenuItem;
   ...
   // procedure SpeedButton6Click(Sender: TObject);
    ...
    //procedure NonUSAregions1Click(Sender: TObject);
    //procedure Extractpointsalongprofile1Click(Sender: TObject);
    ...
    //procedure NLCDgrid1Click(Sender: TObject);
    //procedure VegetationobstaclegridfromNLCD1Click(Sender: TObject);
   // procedure Vegetationobstaclegridfromshapefile1Click(Sender: TObject);
    ...
    //procedure Copyprojection1Click(Sender: TObject);
    ...
    //procedure NLCD20061Click(Sender: TObject);
    ...
    //procedure NLCD2011grid1Click(Sender: TObject);
    //procedure NLCD20111Click(Sender: TObject);
   ...
   // procedure Bandcorrelation1Click(Sender: TObject);
    ...
    procedure Strobl10ktiles1Click(Sender: TObject);
 ...
 public
    { Public declarations }
     ...
     HighZ,LowZ,xDEMg2,yDEMg2,xDEMg1,yDEMg1 : float64;
     ...
     FeaturesDB,
     ScratchDEM,
     ExtremeZDEM,LastX,LastY      : integer;
     SavedMapImage,
...
var
   DEMWanted,i,x,y : integer;
   bb : sfBoundBox;
begin
   if (MapDraw.DEMonMap = 0) then exit;
   i := 0;
   while GetDEM(DEMWanted,true,'Outline grid cells') do begin
      inc(i);
      bb := DEMGlb[DEMWanted].PartOfDEMonMap(Self);
      for x := round(bb.xmin) to round(bb.xmax) do
         for y := round(bb.ymin) to round(bb.ymax) do begin
            if DEMGlb[DEMWanted].DEMHeader.DEMUsed = ArcSecDEM then OutlineGeoBox(DEMGlb[DEMWanted].PixelBoundBoxGeo(x,y),WinGraphColors[i],2)
            else OutlineUTMBox(DEMGlb[DEMWanted].PixelBoundBoxUTM(x,y),WinGraphColors[i],2);
         end;
   end;
...
begin
   ...
   EndProgress;
   DEMGlb[ProbDEM].SetUpMap(ClassDEM,true,mtElevSpectrum,DEMGlb[ClassDEM].AreaName);
   DEMGlb[ProbDEM].SetUpMap(ProbDEM,true,mtElevSpectrum,DEMGlb[ProbDEM].AreaName);
end;
...
begin
   ...
   if ClosingIsHappening or (MapDraw = Nil) or (DEMNowDoing in [Calculating]) or (not MapDraw.MapDrawValid)or ClosingMapNow then exit;
...
var
   ...
   x,y,i  : integer;
begin
  if MapRedrawsAllowed  and (not ClosingMapNow) then begin
...
var
   ...
   fName : ShortString;
   TStr : ShortString;
   OneRec,OtherRec,xg,yg,PathDistance,PathCost,Lat2,Long2,Distance,Azimuth : float64;
begin
...
var
   ...
   TStr : ShortString;
      ...
      begin
        ...
        // with FlightControlForm do  begin
...
var
   ...
   wx,wy : float64;
         ...
         begin
            ...
            DEMGlb[LogDEM].SetUpMap(LogDEM,true,mtElevSpectrum,DEMGlb[CurDEM].AreaName + ' ' + DEMGlb[LogDEM].AreaName);
...
begin
   ...
   for i := 1 to MaxDEMDataSets do begin
      DoDEM[i] := (i <> MapDraw.DEMonMap) and ValidDEM(i);
   end;
   for i := 1 to MaxDEMDataSets do if DoDEM[i] then begin
      NewDEM := MakeDifferenceMapOfBoxRegion(MapDraw.DEMonMap,i,DEMGlb[MapDraw.DEMonMap].FullDEMGridLimits,true,false,false,'Delta ' + DEMGlb[i].AreaName);
...
begin
   ...
   for Col := 0 to pred(DEMGlb[Map1].DEMheader.NumCol) do
   begin
      UpdateProgressBar(Col/DEMGlb[Map1].DEMheader.NumCol);
      for Row := 0 to pred(DEMGlb[Map1].DEMheader.NumRow) do
      begin
         x := 0;
         y := 0;
...
begin
   {$IfDef ExGeoStats}
   {$Else}
      if (MapDraw.DEMonMap <> 0) then SingleDEMHistogram(MapDraw.DEMonMap,true);
   {$EndIf}
end;
             ...
             if not FileExists(fName) then begin
                ...
                if (AntDEM <> 0) then begin
                   if (DecFactor > 1) then begin
                      DEMGlb[AntDEM].ThinThisDEM(ThinDEM,DecFactor);
         ...
         begin
             ...
             if MapDraw.DEMMap and (WhatWanted in [SubsetByOutline,SubsetHole,SubsetLake,ReplaceValuesByOutline,FillHolesByOutline,CalculateVolume]) then begin
                for Col := 0 to pred(Col1) do
                   for Row := 0 to pred(DEMGlb[MapDraw.DEMonMap].DEMheader.NumRow) do DEMGlb[MapDraw.DEMonMap].SetGridMissing(Col,Row);
                for Col := succ(Col2) to pred(DEMGlb[MapDraw.DEMonMap].DEMheader.NumCol) do
                   for Row := 0 to pred(DEMGlb[MapDraw.DEMonMap].DEMheader.NumRow) do DEMGlb[MapDraw.DEMonMap].SetGridMissing(Col,Row);
                for Row := 0 to pred(Row1) do
                   for Col := 0 to pred(DEMGlb[MapDraw.DEMonMap].DEMheader.NumCol) do DEMGlb[MapDraw.DEMonMap].SetGridMissing(Col,Row);
                for Row := succ(Row2) to pred(DEMGlb[MapDraw.DEMonMap].DEMheader.NumRow) do
                   for Col := 0 to pred(DEMGlb[MapDraw.DEMonMap].DEMheader.NumCol) do DEMGlb[MapDraw.DEMonMap].SetGridMissing(Col,Row);
             end;
...
begin
   ...
   if SkipMenuUpdating or Help1.Visible then exit;
   ...
   //end of turned on here, but might be turned off later for small maps
      ...
      Missingdatacolor1.Visible := (MapDraw.DEMonMap <> 0) or (MapDraw.SatonMap <> 0);
    ...
    //Cartography menu
      ...
      ConvertUKOSDEM1.Visible := ExpertDEMVersion and (MapDraw.DEMonMap <> 0) and (DEMGlb[MapDraw.DEMonMap].DEMheader.DigitizeDatum = UK_OS_grid);
   ...
   bmpMem : tBMPMemory;
      ...
      begin
        ...
        while not GISdb[db].MyData.Eof do begin
           if GISdb[db].ValidLatLongFromTable(Lat,Long) then begin
              MapDraw.LatLongDegreeToScreen(Lat,Long,xg,yg);
              if MapDraw.OnScreen(xg,yg) then BMPMem.SetPixelColor(xg,yg,claRed);
           end;
           GISdb[db].MyData.Next;
        end;
      end;
      ...
      begin
         if AnswerIsYes('UTM grid') then begin
            ...
            HeatGrid := CreateGridToMatchMap(cgUTM,true,WordDEM,GridSize,MapDraw.PrimMapProj.projUTMZone);
         end
         else begin
            ReadDefault('Heat map grid size (arc sec)',GridSize);
            HeatGrid := CreateGridToMatchMap(cgLatLong,true,WordDEM,GridSize/3600,MapDraw.PrimMapProj.projUTMZone);
         end;
...
begin
   if (TheMap <> 0) then begin
      VectorMap[TheMap].ClosingMapNow := true;
...
begin
   ...
   if (MapDraw.MapOwner in [moNone,moPointVerificationMap]) then begin
      ...
      {$IfDef RecordClosing} WriteLineToDebugFile('TMapForm close without checking'); if Closable then WriteLineToDebugFile('   closable'); {$EndIf}
       ...
       {$IfDef RecordClosing} WriteLineToDebugFile('MapOwner in [moDEMSelectionMap,moEditMap'); {$EndIf}
       CanClose := false;
       {$IfDef ExMultiGrid}
          CloseSingleDEM(MapDraw.DEMonMap);
       {$Else}
          ...
          else CloseSingleDEM(MapDraw.DEMonMap);
       {$EndIf}
    end
       ...
       {$Else}
          if (MapDraw.MapOwner = moImageSelectionMap) and MapDraw.ValidSatOnMap then begin
             {$IfDef RecordClosing} WriteLineToDebugFile('MapOwner = moImageSelectionMap'); {$EndIf}
             CanClose := false;
   ...
   {$IfDef RecordClosing} WriteLineToDebugFile('Exit TMapForm.FormCloseQuery fallthrough'); {$EndIf}
end;
...
begin
   {$IfDef RecordClosing} WriteLineToDebugFile('Enter TMapForm.formClose for ' + Caption); {$EndIf}
   ...
   If (NumOpenMaps = 1) then begin
      ...
      for i := 1 to MaxDataBase do begin
         if (ValidDB(i)) then begin
            {$IfDef RecordClosing} WriteLineToDebugFile('TMapForm.formClose Close db=' + IntToStr(i)); {$EndIf}
   ...
   {$IfDef RecordClosing} WriteLineToDebugFile('TMapForm.formClose Tdb_display_opts'); {$EndIf}
   for i := 0 to pred(WMDEM.MDIChildCount) do begin
   ...
   {$IfDef RecordClosing} WriteLineToDebugFile('TMapForm.formClose MapDraw'); {$EndIf}
   ...
   {$IfDef RecordClosing} WriteLineToDebugFile('TMapForm.formClose WMDEM'); {$EndIf}
   if (WMDEM <> Nil) then begin
      if (not LockStatusBar) then for i := 0 to 3 do WmDEM.SetPanelText(i, '');
      WmDem.SetMenusForVersion;
   end;

   if (ZoomWindow <> Nil) and (ZoomWindow.Handle = Self.Handle) then ZoomWindow := Nil;
   Self := Nil;
   {$IfDef RecordClosing} WriteLineToDebugFile('Closed map window ' + Caption); {$EndIf}
end;
...
begin
   with MapDraw do begin
      ...
      else if DEMMap then DEMGlb[DEMonMap].VerifyDefaultPosition(WhatFor,xg,yg,xutm,yutm)
      {$IfDef ExSat}
              ;
      {$Else}
      else begin
          SatImage[SatOnMap].SatGridToLatLongDegree(SatImage[MapDraw.SatOnMap].BandForSize,xg,yg,Lat,Long);
          GetLatLongDefault(SatImage[SatOnMap].ImageMapProjection,WhatFor,Lat,Long);
          SatImage[SatOnMap].LatLongDegreeToSatGrid(SatImage[MapDraw.SatOnMap].BandForSize,Lat,Long,xg,yg);
          SatImage[SatOnMap].SatGridToUTM(SatImage[MapDraw.SatOnMap].BandForSize,xg,yg,xutm,yutm);
      end;
      {$EndIf}
   end;
end;
...
begin
   if ClosingIsHappening or LockStatusBar or ClosingMapNow or (WMDEM = Nil) or (MapDraw = Nil) or (Not MapDraw.MapDrawValid) or (DEMNowDoing in [Calculating]) then exit;
   ...
   if ((MapDraw.ValidDEMMap) or MapDraw.ValidDEMonMap) and (not LockStatusBar) then begin
      ...
      if not ((DEMGlb[MapDraw.DEMonMap].DEMheader.DigitizeDatum in [Rectangular])) then
         DEMGlb[MapDraw.DEMonMap].LatLongDegreeToDEMGrid(Lat,Long,XGrid,YGrid);

      if MDDef.ShowDEMGridCoords and (Panel3String = '') then Panel3String := 'DEM grid: x=' + RealToString(Xgrid,-8,1) + '  y=' +  RealToString(Ygrid,-8,1) + ' ' + Panel3String;
...
var
   ...
   Lat2,Long2,Lat3,Long3,{Slope,}xgrid,ygrid,
...
begin
   ...
   if USGSQuadNames1.Checked then begin
      if (not (DEMNowDoing in [ShowMagneticVariation,UTMTrueDeviation,GeodeticBearing,{DoingDatumShift,}NLCDClassification,MapTissotIndicatrix])) and
...
begin
   ...
   with MapDraw do begin
      if (VectorIndex <> 0) then begin
         ScreenToLatLongDegree(X,Y,yDEMg,xDEMg);
         if ValidDEMonMap then DEMGlb[DEMOnMap].LatLongDegreeToDEMGrid(yDEMg,xDEMg,xDEMg,yDEMg);
         exit;
      end;
       ...
       {$Else}
           if (DEMNowDoing = MoveMapBox) then begin
              ...
              with DEMEditForm do begin
                 ...
                 MapDraw.ScreenToDataGrid(NewX1,NewY1,xu,yu);
                 SubSetLeftCol := round(xu);
                 MapDraw.ScreenToDataGrid(NewX2,NewY2,xu,yu);
                 SubsetBottRow := round(yu);
...
begin
   ...
   MonthlyWinds1.Visible := (WindsDB <> 0) and (GISDB[WindsDB] <> nil);
   DrapecurrentmaptoOpenGL1.Visible := MapDraw.DEMonMap <> 0;
...
begin
   ...
   if (dbNum = 0) then begin
      ...
      EditMapViaColor(emvcBasicMask,clBlack);
      DoBaseMapRedraw;
   end
...
var
   ...
   Linear,SaveDEMs{,Spacing} : boolean;
   fName : PathStr;
begin
   ...
   for i := 2 to Levels do begin
      ...
      DEMGlb[MapDraw.DEMonMap].ThinThisDEM(NewDEM,Spacings);
      if (NewDEM <> 0) then begin
         DEMGlb[NewDEM].SetUpMap(NewDEM,true,MapDraw.MapType);
         fName := WriteDEMDir + DEMGlb[NewDEM].AreaName + '.dem';
         if SaveDEMs then DEMGlb[NewDEM].WriteNewFormatDEM(fName);
      end;
      DEMGlb[MapDraw.DEMonMap].ThinThisDEM(NewDEM,Spacings,true);
      if (NewDEM <> 0) then begin
         DEMGlb[NewDEM].SetUpMap(NewDEM,true,MapDraw.MapType);
         fName := WriteDEMDir + DEMGlb[NewDEM].AreaName + '.dem';
         if SaveDEMs then DEMGlb[NewDEM].WriteNewFormatDEM(fName);
...
begin
   if SizingWindow or ClosingMapNow or (MapDraw = Nil) then exit;
   SizingWindow := true;
   if (Sender = Nil) then begin
      SetClientHeight;
   end;
...
begin
   if GetDEM(MergeDEM,true,'subract from ' + DEMGlb[MapDraw.DEMonMap].AreaName) then begin
      DEMGlb[MapDraw.DEMonMap].SubtractaDEM(MergeDEM);
      RespondToChangedDEM;
   end;
end;
...
begin
   fName := DEMGlb[MapDraw.DEMOnMap].VATFileName;
   if FileExists(fName) then begin
      {$IfDef RecordVAT} WriteLineToDebugFile('TMapForm.GridVATlegend1Click found=' + fName); {$EndIf}
   end
   else begin
      fName := DEMGlb[MapDraw.DEMOnMap].DEMFileName + '.vat.dbf';
      if not FileExists(fName) then fName := ChangeFileExt(DEMGlb[MapDraw.DEMOnMap].DEMFileName,'.vat.dbf');
      if not FileExists(fName) then fName := DEMGlb[MapDraw.DEMOnMap].DEMFileName + '.vat.csv';
      if not FileExists(fName) then exit;
      DEMGlb[MapDraw.DEMOnMap].VATFileName := fName;
   ...
   Cat := 0;
   CreateBitmap(bmp,1200,25*succ(Cats) + 4);
   ...
   while not Table.eof do begin
      ...
      bmp.Canvas.Rectangle(5,(Cat)*25,25,succ(Cat)*25);
      bmp.Canvas.Brush.Style := bsClear;
      bmp.Canvas.TextOut(30,Cat*25 + 4, RealToString(xg,12,4) + '  ' + Table.GetFieldByNameAsString(NameField));
      Table.Next;
   end;
   bmp.Height := succ(Cats)*25 + 4;
   PetImage.GetImagePartOfBitmap(bmp);
   Petimage_form.DisplayBitmap(bmp,DEMGlb[MapDraw.DEMOnMap].AreaName + ' Legend');
   ShowDefaultCursor;
   Table.Destroy;
end;
...
begin
   with MapDraw do begin
      ...
      Elevation3.Visible := (MapDraw.DEMonMap <> 0);
...
begin
   if GetDEM(MergeDEM,true,'add to ' + DEMGlb[MapDraw.DEMonMap].AreaName) then begin
      DEMGlb[MapDraw.DEMonMap].AddaDEM(MergeDEM);
      RespondToChangedDEM;
   end;
end;
...
begin
   ...
   if ClosingMapNow or (MapDraw = Nil) or (DEMNowDoing in [Calculating]) or (not MapDraw.MapDrawValid) then exit;
   MouseIsDown := false;
   if MapDraw.DEMMap then begin
   ...
   Data : array[1..MaxDEMDataSets] of float32;
      ...
      begin
         if (DEM <> 0) then  begin
            ...
            DEMGlb[DEM].SetUpMap(DEM,true,mtElevSpectrum,theName);
         end;
      end;
         ...
         for x := 0 to pred(DEMheader.NumCol) do begin
            ...
            for y := 0 to pred(DEMheader.NumRow) do begin
               ...
               if (MomentVar.Npts > 0) then begin
                  ...
                  if MDDef.doSTDDEM and (MomentVar.NPts > 1) then DEMGlb[STDDEM].SetGridElevation(x,y,MomentVar.sdev);
...
begin
   EditMapViaColor(emvcAreaofsinglecolor,clBlack);
end;
...
begin
   ...
   {$IfDef RecordEditsDEM} WriteLineToDebugFile('Grid being masked: ' + DEMGlb[MapDraw.DEMonMap].AreaName); {$EndIf}
   {$IfDef RecordEditsDEM} WriteLineToDebugFile('     Masking grid: ' + DEMGlb[SecondGrid].AreaName); {$EndIf}

   StartProgress('Mask');
   EditsDone := 0;
   ParallelRowsDone := 0;
   PartLimits := GetLimitsForParallelLoops(DEMGlb[MapDraw.DEMonMap].FullDEMGridLimits);

   MaskStripFromSecondGrid(DEMGlb[MapDraw.DEMonMap].FullDEMGridLimits,MapDraw.DEMonMap,SecondGrid,HowMask);
...
(*
   ...
   {$Else}
      TParallel.For(1,MDdef.MaxThreadsForPC,
          procedure (Value: Integer)
          begin
             MaskStripFromSecondGrid(PartLimits[Value],MapDraw.DEMonMap,SecondGrid,HowMask);
          end);
    {$EndIf}
*)
   EndProgress;
...
begin
   EditMapViaColor(emvcFlattenLake,clBlack);
end;
...
var
   ...
   procedure Figure(What : shortstring);
   var
      z : float32;
      i : integer;
   begin
      sl.Add(What + ': ' + LatLongDegreeToString(RightClickLat,RightClickLong,MDDef.OutPutLatLongMethod));
      if (MapDraw.DEMonMap <> 0) and DEMGlb[MapDraw.DEMonMap].GetElevFromLatLongDegree(RightClickLat,RightClickLong,z) then sl.Add(RealToString(z,12,4) + ' ' + LabelElevUnits[DEMGlb[MapDraw.DEMonMap].DEMHeader.ElevUnits] + '   ' + DEMGlb[MapDraw.DEMonMap].AreaName);

      for i := 1 to MaxDEMDataSets do begin
         if ValidDEM(i) and (i <> MapDraw.DEMonMap) and DEMGlb[i].GetElevFromLatLongDegree(RightClickLat,RightClickLong,z) then sl.Add(RealToString(z,12,4)+ ' ' + LabelElevUnits[DEMGlb[i].DEMHeader.ElevUnits] + '   ' + DEMGlb[i].AreaName);
      end;
...
var
  ...
  OK,Prime : boolean;
  xg1,yg1,xg2,yg2,xs,ys, xg,yg,lat,Long,xc,yc,LatC,LongC,Lat1,Long1,h,k : float64;
begin
  with mapDraw do begin
     MapDraw.ScreenToLatLongDegree(lastx,LastY,Lat,Long);
...
begin
   ...
   for i := 1 to MDDef.PyramidLevels do begin
      ...
      DEMGlb[MapDraw.DEMonMap].ThinThisDEM(ThinDEM[i],tf);
      DEMGlb[ThinDEM[i]].AreaName := 'thin_' + IntToStr(i);
      MapHouseKeeping(ThinDEM[i]);

      DEMGlb[MapDraw.DEMonMap].ThinThisDEM(MeanDEM[i],tf,true);
      DEMGlb[MeanDEM[i]].AreaName := 'mean_' + IntToStr(i);
   ...
   for i := 1 to MDDef.PyramidLevels do begin
      ...
      DEMGlb[FilDEM].ThinThisDEM(DownDEM[i],2);
      CloseSingleDEM(FilDEM);
      DEMGlb[DownDEM[i]].AreaName := 'down_' + IntToStr(i);
...
begin
   ...
   if (not ClosingMapNow) then DoFastMapRedraw;
   VariableOpaqueOverlays := false;
end;
...
begin
   ...
   NewHeader.LatHemi := MapDraw.PrimMapProj.LatHemi;
    ...
    else if MapDraw.IsThisMapUTM and (MapDraw.DEMonMap <> 0) then begin
    ...
    if OpenMap then begin
       DEMGlb[Result].SetUpMap(Result,false,MDDef.DefDEMMap,DEMGlb[Result].AreaName);
...
begin
   ...
   fName := MDTempDir + 'horiz_shift_' + DEMGlb[MapDraw.DEMonMap].AreaName + '.dem';
   DEMGlb[MapDraw.DEMonMap].WriteNewFormatDEM(fname);
...
var
   ...
   ch1,ch2 : char;
begin
   for Long := round(MapDraw.MapCorners.BoundBoxGeo.XMin-1) to round(MapDraw.MapCorners.BoundBoxGeo.XMax+1) do begin
      if Long >= 0 then ch2 := 'e' else ch2 := 'w';
      for Lat := round(MapDraw.MapCorners.BoundBoxGeo.YMin-1) to round(MapDraw.MapCorners.BoundBoxGeo.YMax+1) do begin
...
procedure TMapForm.Nightlights20161Click(Sender: TObject);
begin
   {$IfDef Nevadia}
   {$Else}
      wmdem.Nightlights1Click(Sender);
   {$EndIf}
end;
...
var
   fName : PathStr;
begin
   if (DEMGlb[MapDraw.DEMonMap].DEMHeader.RasterPixelIsGeoKey1025 in [2]) then begin
      ...
      DEMGlb[MapDraw.DEMonMap].DEMHeader.RasterPixelIsGeoKey1025 := 1;
...
(*
         if DEMHeader.RasterPixelIsGeoKey1025 in [0,1] then begin
            //area or undefined
            DEMHeader.DEMSWCornerX := DEMHeader.DEMSWCornerX - 0.5 * DEMHeader.DEMxSpacing;
            DEMHeader.DEMSWCornerX := DEMHeader.DEMSWCornerY - 0.5 * DEMHeader.DEMySpacing;
...
var
   fName : PathStr;
begin
   if (DEMGlb[MapDraw.DEMonMap].DEMHeader.RasterPixelIsGeoKey1025 in [1]) then begin
      DEMGlb[MapDraw.DEMonMap].DEMHeader.DEMSWCornerX := DEMGlb[MapDraw.DEMonMap].DEMHeader.DEMSWCornerX + 0.5 * DEMGlb[MapDraw.DEMonMap].DEMHeader.DEMxSpacing;
      DEMGlb[MapDraw.DEMonMap].DEMHeader.DEMSWCornerX := DEMGlb[MapDraw.DEMonMap].DEMHeader.DEMSWCornerY + 0.5 * DEMGlb[MapDraw.DEMonMap].DEMHeader.DEMySpacing;
      DEMGlb[MapDraw.DEMonMap].DEMHeader.RasterPixelIsGeoKey1025 := 2;
...
procedure TMapForm.EditMapViaColor(emvc : temvc; Color : tColor; LakeZ : float64 = -MaxSmallInt; SureOfColor : boolean = false; ShowResults : boolean = true);
var
...
begin
    ...
    if SureOfColor or (emvc in [emvcBasicMask,emvcSetElevation]) or ColorDialog1.Execute then begin
       ...
       CopyImageToBitmap(Image1,Bitmap);
       {$IfDef RecordColorMasking}
       WriteLineToDebugFile('copied mask to temp directory');
       Bitmap.SaveToFile(MDTempDir + 'Selectedcolor1-map.bmp');
       {$EndIf}
...
begin
   EditMapViaColor(emvcSelectedColor,-99);
end;
...
begin
   EditMapViaColor(emvcAllbutselectedcolor,-99);
end;
...
begin
   ...
   if (Sender = DetrendDEMgrid1) then DEMGlb[MapDraw.DEMonMap].DetrendDEM(NewDEM);
