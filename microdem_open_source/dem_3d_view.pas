...
type
  tView3D = class
     ...
     {for oblique, corner point order is:
        ...
        Findfxgrids,Findfygrids,Findfdists : ^Petmath.bfarray32;
        constructor Create;
        destructor Destroy;
...
var
   ...
   elevs,fxgrids,fygrids,fdists : ^Petmath.bfarray32;
...
begin
   ...
   tanMaxElevAngle := tanDeg(MaxPitch);
    ...
    if (NumRadialPts > PetMath.MaxFArrayPts) then NumRadialPts := pred(PetMath.MaxFArrayPts);
   ...
   if (MDdef.PerspOpts.PersFirstProfile < 1) then MDdef.PerspOpts.PersFirstProfile := 1;
    ...
    for Runs := 1 to NumRuns do begin
       ...
       if (PersOpts.WhichPerspective in [BMPPerspective,ReflectancePerspective]) then begin
          ...
          if (SpecifiedCaption = '') then ViewCaption := DEMGlb[DEMonView].DEMMapProjection.PreferLocationString(ViewerLat,ViewerLong) + '    ' + TStr
