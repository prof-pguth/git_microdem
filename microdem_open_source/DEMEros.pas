...
var
   ...
   begin
      ...
      {$Else}
         ...
         if GDALImageFormat(Ext) then begin
            ...
            if FileExists(IndexFileName) then begin
               ...
               Result := false;
               MessageToContinue('GDAL failure; run ' + MDtempDir + 'rev2.bat in command window to see why');
            end;
         end;
         {$EndIf}
      end;
      ...
      begin
         ...
         {$IfDef ExTIN} {$Else} SatTINRegistration := Nil; {$EndIf}
         {$IfDef ExAdvancedSats} {$Else} HypersectralImage := Nil; {$EndIf}
         {$IfDef NoBMPFileImagery} {$Else} BMPFile := false; {$EndIf}
      end;
