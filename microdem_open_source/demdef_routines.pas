...
function DEMGridString(xgrid,ygrid : float32; Decs : integer = 2) : shortstring;
...
function ZUnitCategory (Zunit : tElevUnit) : shortstring;
begin
   if zunit in [euMeters,euDecimeters,euFeet,euDeciFeet,euCentimeters] then Result := 'Elevation'
   else if zunit in [euPercentSlope] then Result := 'Slope'
   else Result := 'Grid';
end;
...
function DEMGridString(xgrid,ygrid : float32; Decs : integer = 2) : shortstring;
begin
   Result := 'DEM grid: x=' + RealToString(Xgrid,-8,Decs) + '  y=' +  RealToString(Ygrid,-8,Decs);
end;
...
begin
   Result := (MapType in [mtDEMContour,mtDEMaspect,mtDEMaspectSlope,mtDEMBlank,mtMergeTwoDEMs,mtDEMMask,mtDEMVATTable,
        mtFlowDir360,mtFlowDirArc,mtFlowDirTau,mtDEMContour,mtDEMReflectElevMerge,mtNoChange, mtDEMMask,mtOpenness,mtLASclass,mtRGBimagery,mtLandCover,mtCurvature,mtDifferenceDiverge]);
end;
