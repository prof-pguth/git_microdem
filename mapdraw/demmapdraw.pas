...
//end needed for inline core DB functions
   ...
   {$If Defined(NoParallelForMaps) and Defined(NoParallelLAS) and Defined(NoParallelOpenness)}
   {$Else}
      System.Threading,
   {$EndIf}
...
type
   ...
   public
     ...
     PrimMapProj,SecMapProj : tMapProjection;
     ...
     //map coordinate routines
      ...
      procedure ScreenToDEMGrid(xpic,ypic : integer; var XGrid,YGrid : float64);  {$IfDef InlineCoreMapping} inline; {$EndIf}
      procedure ScreenToDEMGridInteger(xpic,ypic : integer; var XGrid,YGrid : integer);  {$IfDef InlineCoreMapping} inline; {$EndIf}
   ...
   //onscreen boolean functions
      ...
      function CreateMapPerimeterDB : integer;
   ...
   //world file routines
      ...
      function GridLimitsOfSecondDEMonMap(DEM2 : integer) : tGridLimits;
...
function tMapDraw.CreateMapPerimeterDB : integer;
var
   csv : tStringList;
   i : integer;
   fName : PathStr;
         ...
         if DrawBaseMap then begin
            ...
            if (BaseMapFName <> '') and FileExists(BaseMapFName) then begin
               ...
               if (VectorIndex <> 0) then begin
                  ...
                  else if IsReflectanceMap(MapType) or IsElevationMap(MapType) or (MapType in [mtDEMVATTable,mtLandCover,mtCurvature,mtDifferenceDiverge]) then begin
                     {$IfDef ExVegDensity}
                     {$Else}
...
begin
   ...
   NewMapDraw.PrimMapProj := PrimMapProj;
   NewMapDraw.SecMapProj := SecMapProj;
...
{$Else}
   ...
   var
      ...
      {$If Defined(NoParallelLAS)} {$Else} BaseStart : integer; {$EndIf}
...
begin
   if (fName = '') then fName := NextFileNumber(MDTempDir, BaseTitle + '_map_layer', OverlayFExt);
         ...
         DoneOne : boolean;
               ...
               begin
                 ...
                 if DoneOne or AddFirst then begin
                    TStr := '';
                    if ValidDEM(DEM) and DEMGlb[DEM].GetElevFromLatLongDegree(Lat,Long,z) then TStr := RealToString(z,-8,-2);
            ...
            try
               ...
               for y := 0 to pred(MapYSize) do begin
                  ...
                  for x := 0 to pred(MapXSize) do begin
                     ScreenToDEMGridInteger(x,y,XG1,YG1);
