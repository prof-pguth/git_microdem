...
type
  Tdbtablef = class(TForm)
   ...
   //Wins1: TMenuItem;
    ...
    Differentrankingsbytile1: TMenuItem;
    ...
    DTMDSMcomparison1: TMenuItem;
    DTDSMcomparison1: TMenuItem;
    AddDSMDTMpair1: TMenuItem;
    IdentifyDSMandDTM1: TMenuItem;
    N61: TMenuItem;
    GraphscompringDSMDTMslopes1: TMenuItem;
    GraphscompartingDSMDTMdifferencedistribtuions1: TMenuItem;
    CompareDSMDTM1: TMenuItem;
    DSMlessthanDTM1: TMenuItem;
    ...
    Addcanopyheight1: TMenuItem;
    Addbiome1: TMenuItem;
    AddKoppen1: TMenuItem;
    Graphinterpolationresults1: TMenuItem;
    ...
    N62: TMenuItem;
    ListuniquefieldvaluesallopenDB1: TMenuItem;
    DEMIX3: TMenuItem;
    Addcentroid1: TMenuItem;
    Xaxis1: TMenuItem;
    Yaxis1: TMenuItem;
    PowerlawforHRDEMDSMDTM1: TMenuItem;
    Graphslopes1: TMenuItem;
    OpenHRDEMDSMDTMLandcover1: TMenuItem;
    HRDEMslopeandslopedifferencemaps1: TMenuItem;
    LoadtestandreferenceDEMs2: TMenuItem;
    ...
    BestreferenceDEMforALOS1: TMenuItem;
    AddDSMDTM1m1: TMenuItem;
    BestrefrenceDEMs1: TMenuItem;
    Both2: TMenuItem;
    GraphwheretilebestrepresentsDSMorDTM1: TMenuItem;
    MapcomparingtestDEMtoDSMandDTM1: TMenuItem;
    ...
    procedure AddDSMDTMpair1Click(Sender: TObject);
    ...
    procedure Addcanopyheight1Click(Sender: TObject);
    procedure Addbiome1Click(Sender: TObject);
    procedure AddKoppen1Click(Sender: TObject);
    procedure Graphinterpolationresults1Click(Sender: TObject);
    ...
    procedure ListuniquefieldvaluesallopenDB1Click(Sender: TObject);
    procedure Addcentroid1Click(Sender: TObject);
    procedure Yaxis1Click(Sender: TObject);
    procedure Xaxis1Click(Sender: TObject);
    procedure PowerlawforHRDEMDSMDTM1Click(Sender: TObject);
    procedure Graphslopes1Click(Sender: TObject);
    procedure OpenHRDEMDSMDTMLandcover1Click(Sender: TObject);
    procedure HRDEMslopeandslopedifferencemaps1Click(Sender: TObject);
    ...
    procedure BestreferenceDEMforALOS1Click(Sender: TObject);
    procedure AddDSMDTM1m1Click(Sender: TObject);
    procedure Both2Click(Sender: TObject);
    procedure GraphwheretilebestrepresentsDSMorDTM1Click(Sender: TObject);
    procedure CompareCOPtopointandareareferenceDEMs1Click(Sender: TObject);
    procedure MapcomparingtestDEMtoDSMandDTM1Click(Sender: TObject);
  private
...
procedure OpenHRDEMforCurrentRecord(db : integer; var DTM,DSM,LCgrid : integer);
var
...
begin
  Result := (FieldName = 'NAME') or (FieldName = 'DEM_NAME') or (FieldName = 'EVT_NAME') or (FieldName = 'NAME00') or (FieldName = 'NAME10') or
       (FieldName = 'F_NAME') or (FieldName = 'LONG_NAME') or (FieldName = 'PALETTE') or (FieldName = 'SENSORS') or (FieldName = 'DEMIX_TILE') or
       (FieldName = 'PLACE') or (FieldName = 'CLASS_NAME') or (FieldName = 'CLASS') or (FieldName = 'CLUSTER') or (FieldName = 'GROUP') or (FieldName = 'COLOR');
end;
...
begin
    ...
    if (Operation = sfaAbsoluteValue) then begin
       Mult := -1;
    ...
    while not GISdb[DBonTable].MyData.EOF do begin
       ...
       if GISdb[DBonTable].GetFloat32FromTableLink(CheckField,Value) then begin
          ...
          else if (Operation = sfaAbsoluteValue) then Value := abs(Value)
...
procedure Tdbtablef.OpenHRDEMDSMDTMLandcover1Click(Sender: TObject);
var
   DTM,DSM,LCgrid : integer;
begin
   OpenHRDEMforCurrentRecord(dbOnTable,DTM,DSM,LCgrid);
end;
...
procedure Tdbtablef.Xaxis1Click(Sender: TObject);
begin
   CreateMultipleSeriesGraph(dbOnTable,'',nil,true);
end;
...
procedure Tdbtablef.Yaxis1Click(Sender: TObject);
begin
   CreateMultipleSeriesGraph(dbOnTable,'',nil,false);
end;
...
*)
        ...
        if PictureInDBField(aField) then begin
...
begin
   ...
   with GISdb[DBonTable] do begin
      ...
      DTMDSMcomparison1.Visible := DEMIX_HRDEM_DB(dbOnTable) or DEMIX_GDEM_DB(dbOnTable);
      DEMIX2.Visible := GISdb[DBonTable].IsThisDEMIXdatabase;
...
begin
   if ValidDB(DBonTable) then begin
      ...
      //SummarybestCOPreferenceDEM1.Enabled := GISdb[DBonTable].MyData.FieldExists('BEST_ALL');
      BestreferenceDEMforCOP1.Enabled := CheckDEMIXdbType(dbOnTable,dbType,[ddtTileStats],'');
      BestreferenceDEMforALOS1.Enabled := BestreferenceDEMforCOP1.Enabled;
      GraphwheretilebestrepresentsDSMorDTM1.Visible := GISdb[DBonTable].MyData.FieldExists('WHO_TIES') and GISdb[DBonTable].MyData.FieldExists('DSM_OR_DTM');
      DEMIX1Click(Sender);
   end;
end;
...
begin
   ...
   if ValidDB(DBonTable) then GISdb[DBonTable].DisplayTable(GISdb[DBonTable].MyData.Filter);
   ShowStatus;
end;
...
begin
   if ValidDB(DBonTable) and (not FormWorking) and (not Closing) then begin
      {$IfDef RecordFormActivate} WriteLineToDebugFile('Tdbtablef.FormActivate in ' + GISdb[DBonTable].DBname); {$EndIf}
...
var
  ...
  fName : PathStr;
      ...
      begin
         ...
         GridForm.Caption := GISdb[DBonTable].dbName + '  ' + LongFilter + GISdb[DBonTable].NumRecsInDBstring + '  unique entries=' + IntToStr(pred(Findings.Count));
   ...
   with GISdb[DBonTable] do begin
     ...
     if (WantedFieldName <> '') then begin
        ...
        if (Sender = Singlefield1) or (Sender = Frequencytable1) or (Sender = Assignuniquecolors1) then begin
           ...
           Findings := tStringList.Create;
           if (Sender = Assignuniquecolors1) then Findings.Add(WantedFieldName + ',N,COLOR')
...
procedure Tdbtablef.ListuniquefieldvaluesallopenDB1Click(Sender: TObject);
var
   FullList,SingleList : tStringList;
   i,j : integer;
begin
   {$IfDef RecordCSVOut} WriteLineRoDebugFile('ListuniquefieldvaluesallopenDB in, field = ' + SelectedColumn); {$EndIf}
   SetColorForProcessing;
   if SelectedColumn = '' then SelectedColumn := GISdb[DBonTable].PickField('for unique entries',[ftString]);

   GISdb[DBonTable].EmpSource.Enabled := false;
   FullList := GISdb[DBonTable].MyData.ListUniqueEntriesInDB(SelectedColumn);
   GISdb[DBonTable].EmpSource.Enabled := true;
   for i := 1 to MaxDataBase do if ValidDB(i) and (i <> DBonTable) then begin
      if  GISdb[i].MyData.FieldExists(SelectedColumn) then begin
          {$IfDef RecordCSVOut} WriteLineRoDebugFile(GISdb[i].dbName); {$EndIf}
          GISdb[i].EmpSource.Enabled := false;
          SingleList := GISdb[i].MyData.ListUniqueEntriesInDB(SelectedColumn);
          GISdb[i].EmpSource.Enabled := true;
          for j := 0 to pred(SingleList.Count) do begin
             FullList.Add(SingleList.Strings[j]);
          end;
          SingleList.Destroy;
      end;
   end;
   DisplayAndPurgeStringList(FullList,SelectedColumn +  ' Unique values: ' + IntToStr(FullList.Count));
   SetColorForWaiting;
   {$IfDef RecordCSVOut} WriteLineRoDebugFile('ListuniquefieldvaluesallopenDB out'); {$EndIf}
end;
...
procedure Tdbtablef.Differentrankingsbytile1Click(Sender: TObject);
begin
   DifferentRankingsByTile(DBonTable);
end;
...
procedure Tdbtablef.Both2Click(Sender: TObject);
begin
   FindBestReferenceDEM(dbOnTable,'ALOS');
   FindBestReferenceDEM(dbOnTable,'COP');
end;
...
begin
   ...
   with GISdb[DBonTable] do begin
      ...
      {$IfDef RecordTerrainProfiles} WriteLineToDebugFile('Profile rec start=' + IntToStr(MyData.RecNo) + ' CurrentLinePartSize[1]=' + IntToStr(aShapeFile.CurrentLinePartSize[1])); {$EndIf}
       ...
       if (Sender = CompareshapefileandDEMprofiles1) then Result.OpenPointSymbolFile(rf2,'',Result.MainSymbol);
...
begin
   GISdb[DBonTable].MakeGraph(dbgtN2Dgraph2series);
end;
     ...
     for j := 1 to MaxDataBase do if ValidDB(j) then begin
        if GISdb[j].MyData.FieldExists(XF) and GISdb[j].MyData.FieldExists(YF) then begin
            ...
            if (Sender = N2Dgraphallopendatabases1) and GISdb[j].ColorPresent then begin
               ...
               ThisGraph.OpenXYColorFile(rfile,GISdb[j].dbName);
               ...
               ThisGraph.MainSymbol.Color := ConvertTColorToPlatformColor(WinGraphColors(j));  //GISdb[j].dbOpts.LineColor;
               ThisGraph.MainSymbol.DrawingSymbol := GISdb[j].dbOpts.Symbol.DrawingSymbol;
               ThisGraph.OpenPointSymbolFile(rfile,GISdb[j].dbName,ThisGraph.MainSymbol);
            end
               ...
               ThisGraph.OpenDataFile(rfile,GISdb[j].dbName);
...
procedure Tdbtablef.AbsoluteValue1Click(Sender: TObject);
begin
   SingleFieldArithmetic(DBonTable,sfaAbsoluteValue,SelectedColumn);
end;
...
procedure Tdbtablef.Addbiome1Click(Sender: TObject);
begin
   AddBiomesToDEMIXdb(DBonTable);
end;
...
procedure Tdbtablef.Addcanopyheight1Click(Sender: TObject);
begin
   AddCanopyHeightToDEMIXDB(DBonTable);
end;

procedure Tdbtablef.Addcentroid1Click(Sender: TObject);
begin
   GISdb[dbOnTable].AddCentroidsToFile;
end;
...
begin
   ...
   while not GISdb[DBonTable].MyData.eof do begin
      if GISdb[DBonTable].MyData.ValidLatLongFromTable(Lat,Long) then begin
         tname := GetUTM_DEMIXTile(Lat,Long);
         GISdb[DBonTable].MyData.Edit;
         GISdb[DBonTable].MyData.SetFieldByNameAsString('DEMIX_TILE',tName);
      end;
      GISdb[DBonTable].MyData.Next;
   end;
   ShowStatus;
...
procedure Tdbtablef.AddDSMDTM1m1Click(Sender: TObject);
begin
   DSM_gt_DTM_1(dbOnTable);
end;
...
begin
   ...
   if StrUtils.AnsiContainsText(GISdb[dbOnTable].dbName,'tile_stats') then begin
       IdentifyDSMandDTM1Click(nil);
       FormWorking := true;
       IdentifyDSM_DTMpair(DBonTable);
       FormWorking := false;
       ShowStatus;
   end
   else begin
      MessageToContinue('Not tile stats DB');
   end;
   {$IfDef RecordDEMIX} WriteLineToDebugFile('Tdbtablef.AddDSMDTMpair1Click out'); {$EndIf}
end;


procedure Tdbtablef.AddDSMPCFORESTPCURBANPC1Click(Sender: TObject);
//adds field DSM_PC with percentage of DEM where DSM and DTM should be different, urban and forest landcovers
begin
   SetColorForProcessing;
   FormWorking := true;
   DSM_DTM_DifferFromLandcover(dbOnTable);
   FormWorking := false;
   SetColorForWaiting;
end;
...
procedure Tdbtablef.AddKoppen1Click(Sender: TObject);
begin
   AddKoppenClassToDB(DbOnTable);
end;
...
begin
   with GISdb[DBonTable] do begin
      ShowHourglassCursor;
      Petmar.GetString('String to add end each record',ExtToAdd,false,ReasonableTextChars);
      EmpSource.Enabled := false;
      GISdb[DBonTable].MyData.First;
      while not GISdb[DBonTable].MyData.eof do begin
         aString := GISdb[DBonTable].MyData.GetFieldByNameAsString(SelectedColumn);
         GISdb[DBonTable].MyData.Edit;
         GISdb[DBonTable].MyData.SetFieldByNameAsString(SelectedColumn,aString + extToAdd);
         GISdb[DBonTable].MyData.Next;
      end;
      ShowStatus;
   end;
...
begin
   ...
   //with GISdb[DBonTable] do begin
      ...
      else GISdb[DBonTable].AddXYZfields;
      GISdb[DBonTable].EmpSource.Enabled := false;
      FieldsInDB := Nil;
      FieldsInDB := GISdb[DBonTable].MyData.ListUniqueEntriesInDB(GISdb[DBonTable].dbOpts.LinkFieldThisDB);
      ShowHourglassCursor;
      rc := FieldsInDB.Count;
      ...
      for i := 0 to pred(FieldsInDB.Count) do begin
         UpdateProgressBar(i/rc);
         GISdb[DBonTable].EmpSource.Enabled := false;
         GISdb[DBonTable].dbOpts.MainFilter := GISdb[DBonTable].dbOpts.LinkFieldThisDB + '=' + QuotedStr(trim(FieldsInDB.Strings[i]));
         GISdb[DBonTable].AssembleGISFilter;
         ...
         if (GISdb[DBonTable].LinkTable.RecordCount=0) then begin
            {$IfDef ExportCoords} WriteLineToDebugFile('No bin for ' + FieldsInDB.Strings[i]); {$EndIf}
            MissingBins.Add(FieldsInDB.Strings[i]);
         end
         else if (GISdb[DBonTable].LinkTable.RecordCount < GISdb[DBonTable].MyData.RecordCount) then begin
            ...
            while not GISdb[DBonTable].MyData.eof do begin
               GISdb[DBonTable].MyData.Edit;
               GISdb[DBonTable].MyData.SetFieldByNameAsFloat('X',GISdb[DBonTable].LinkTable.GetFieldByNameAsFloat('X'));
               GISdb[DBonTable].MyData.SetFieldByNameAsFloat('Y',GISdb[DBonTable].LinkTable.GetFieldByNameAsFloat('Y'));
               GISdb[DBonTable].MyData.SetFieldByNameAsFloat('Z',GISdb[DBonTable].LinkTable.GetFieldByNameAsFloat('Z'));
               GISdb[DBonTable].MyData.Next;
               GISdb[DBonTable].LinkTable.Next;
            end;
         end;
      end;
      FieldsInDB.Free;
      GISdb[DBonTable].ClearGISFilter;
      GISdb[DBonTable].LinkTable.ApplyFilter('');
      EndProgress;
      ShowStatus;
...
begin
   //with GISdb[DBonTable] do begin
      ...
      if not ThreeD then begin
         ...
         if GISdb[DBonTable].XYZFile then begin
            GISdb[DBonTable].AddXYZfields;
            GISdb[DBonTable].LatFieldName := 'Y';
            GISdb[DBonTable].LongFieldName := 'X';
         end
         else begin
            ItsLatLong;
            GISdb[DBonTable].AddFieldToDataBase(ftFloat,'Z',18,6);
         end;
      end;
      ...
      while not GISdb[DBonTable].MyData.eof do begin
         ...
         inc(i);
         GISdb[DBonTable].aShapeFile.ReadPointRecord3D(GISdb[DBonTable].MyData.RecNo,PointsHeader);
         GISdb[DBonTable].MyData.Edit;
         GISdb[DBonTable].MyData.SetFieldByNameAsFloat(GISdb[DBonTable].LongFieldName,PointsHeader.x);
         GISdb[DBonTable].MyData.SetFieldByNameAsFloat(GISdb[DBonTable].LatFieldName,PointsHeader.y);
...
begin
   ...
   //with GISdb[DBonTable] do begin
      WantedField := GISdb[DBonTable].PickField('coloring',NumericFieldTypes);
      for i := 1 to MaxDataBase do begin
         if (GISdb[i] <> nil) then begin
...
procedure Tdbtablef.Sortandreplacedatabase2Click(Sender: TObject);
begin
   UnHideColumns;
...
begin
   SortDataBase(DBonTable,true,true,'BEST_EVAL',ExtractFilePath(GISdb[DBonTable].dbFullName));
end;
...
begin
   if (Sender = AllDBsmultiplegraphs1) then begin
       WantXField := GISdb[DBonTable].PickField('Field for histogram',[ftFloat,ftInteger,ftSmallInt]);
             ...
             if AnswerIsYes('Rescale') then begin
               ...
               for i := 1 to MaxDataBase do begin
                  if (Hists[i] <> Nil) then begin
                     ...
                     Hists[i].RedrawDiagram;
                  end;
               end;
             end;
          end
          else GISdb[DBonTable].OldCreateHistogramFromDatabase(true,WantXField,'','',(Sender = AllDBs1));
         end;
   end
   else begin
...
procedure Tdbtablef.HRDEMslopeandslopedifferencemaps1Click(Sender: TObject);
var
   DTM,DSM,LCgrid,DTMslope,DSMslope : integer;
begin
   OpenHRDEMforCurrentRecord(dbOnTable,DTM,DSM,LCgrid);
   if ValidDEM(DTM) and ValidDEM(DSM) then begin
      DTMslope := Make_Grid.CreateEvansSlopeMapPercent(true,DTM);
      DSMslope := Make_grid.CreateEvansSlopeMapPercent(true,DSM);
      MakeDifferenceMap(DSMSlope,DTMSlope,DSMslope,DSM)
   end;
end;
...
begin
   {$IfDef RecordCSVOut} WriteLineRoDebugFile('Tdbtablef.Text1Click in'); {$EndIf}
    ...
    if Petmar.GetFileNameDefaultExt('Text file export','CSV|*.csv|Text|*.txt',fName) then begin
       GISdb[DBonTable].ExportToCSV(fName,ch,tf);
(*
...
begin
  ...
  //ThisGraph.GraphDraw.LegendList := tStringList.create;
   ...
   for j := 0 to pred(subs.Count) do begin
      ...
      PetMath.HeapSort(NPts,zs^);
      ThisGraph.OpenDataFile(rfile,Subs.Strings[j] + '__n=' + IntToStr(NPts));
...
procedure Tdbtablef.CompareCOPtopointandareareferenceDEMs1Click(Sender: TObject);
begin
   MessageToContinue('In progress');
end;
...
begin
   SortDataBase(DBOnTable,true,true);
end;

procedure Tdbtablef.Ascending2Click(Sender: TObject);
begin
   SortDataBase(DBOnTable,true,true,SelectedColumn);
end;
...
begin
   ...
   if CopyImageToBitmap(GISdb[DBonTable].theMapOwner.Image1,Bitmap) then begin
...
begin
   ...
   MessageToContinue('Length: ' + SmartDistanceMetersFormat(GISdb[DBonTable].aShapeFile.LineLength(GISdb[DBonTable].MyData.RecNo)),True );
end;
...
begin
   //with GISdb[DBonTable] do begin
       WantedFieldName := GISdb[DBonTable].PickField('Compass angles',[ftFloat,ftInteger,ftSmallInt]);
       GISdb[DBonTable].EmpSource.Enabled := false;
       GISdb[DBonTable].MyData.First;
       i := 0;
       rc := GISdb[DBonTable].MyData.RecordCount;
   ...
   with GISdb[DBonTable] do begin
      ...
      while not GISdb[DBonTable].MyData.Eof do begin
         ...
         if (Sender = Dayssincefullmoon1) then begin
            {$IfDef ExGeography}
            {$Else}
               if ConstantLocation or ValidLatLongFromTable(Lat,Long) then GISdb[DBonTable].MyData.SetFieldByNameAsInteger('SINCE_FULL',MP_DaysSinceFullMoon(m,d,y,Lat,Long));
            {$EndIf}
         end
...
begin
   DEMlist := nil;
   DisplayBitmap(DEMIXTestDEMLegend(DEMList,Sender <> Vertical1),'Test DEMs');
end;
...
begin
   //with GISdb[DBonTable] do begin
      AddOrSubtractOverlay(GISdb[DBonTable].TheMapOwner,ovoFans,false);
      ...
      if GetWeaponParameters(GISdb[DBonTable].TheMapOwner.MapDraw.DEMonMap,wf,false,false,Nil,true,true) then begin
         if (Sender = ViewshedFields1) then GISdb[DBonTable].AddFieldGroupToTable('WeaponsFan');
         GISdb[DBonTable].MyData.First;
         GISdb[DBonTable].EmpSource.Enabled := false;
         i := 0;
         while not GISdb[DBonTable].MyData.Eof do begin
            ...
            if GISdb[DBonTable].ValidLatLongFromTable(wf.W_Lat,wf.W_Long) then
               AddFanToWeaponsTable(GISdb[DBonTable].TheMapOwner.MapDraw.PrimMapProj,true,true,GISdb[DBonTable].MyData,wf);
            GISdb[DBonTable].MyData.Next;
         end;

         GISdb[DBonTable].ItsFanFile := true;
         GISdb[DBonTable].dbTablef.Panel1.Visible := true;
         ShowStatus;
         GISdb[DBonTable].TheMapOwner.MapDraw.CurrentFansTable := DBonTable;
...
begin
   GISdb[DBonTable].MakeGraph(dbgtN2DgraphcolorNumericField);
end;
...
procedure Tdbtablef.GDEMaveragetileslopebyresolution1Click(Sender: TObject);
begin
...
begin
   //with GISdb[DBonTable] do begin
      GISdb[DBonTable].MyData.Edit;
      if (Sender = EditPointSymbol1) and GISdb[DBonTable].PointSymbolFieldsPresent then begin
         GISdb[DBonTable].MyData.DefinePointSymbol(Sym,SymSize,SymColor);
         GetSymbol(Sym,SymSize,SymColor);
         GISdb[DBonTable].MyData.PostPointSymbol(Sym,SymSize,SymColor);
      end;
      if (Sender = EditFont1) and GISdb[DBonTable].FontFieldExists then begin
         Font := tFont.Create;
...
*)
      ...
      GetLatLongDefault(GISdb[DBonTable].TheMapOwner.MapDraw.PrimMapProj,'revised location',Lat,Long);
      GISdb[DBonTable].MyData.Edit;
...
begin {procedure Tdbtablef.ShowStatus}
   ...
   if (not FormWorking) and (Closing <> true) and ValidDB(DBonTable) then begin
...
begin
   GISdb[DBonTable].MakeGraph(dbgtPlotFieldDifference);
end;
...
begin
    if (GISdb[DBonTable].MyData.RecordCount > 35) and (Not AnswerIsYes('Proceed with ' + IntToStr(GISdb[DBonTable].MyData.RecordCount) + ' records')) then exit;
    GISdb[DBonTable].MyData.First;
    while not GISdb[DBonTable].MyData.eof do begin
...
begin
   ...
   GISdb[DBonTable].ActuallyDrawGraph(dbgtN2Dgraphcolorcodetext);
end;
...
procedure Tdbtablef.Graphinterpolationresults1Click(Sender: TObject);
begin
   if TileCharacteristicsInDB(DBonTable) then begin
      GraphCompareInterpolation(dbOnTable);
   end;
end;
...
procedure Tdbtablef.GraphscompartingDSMDTMdifferencedistribtuions1Click(Sender: TObject);
var
  theTiles : tStringList;
  I : integer;
begin
    GISdb[DBonTable].EmpSource.Enabled := false;
...
begin
    ...
    GISdb[DBonTable].EmpSource.Enabled := false;
    for I := 0 to pred(TheTiles.Count) do begin
       GraphCompareDSMandDTMslopes(DBonTable,TheTiles.Strings[i]);
    end;
    TheTiles.Destroy;
    ShowStatus;
end;
...
procedure Tdbtablef.Graphslopes1Click(Sender: TObject);
begin
    GraphCompareDSMandDTMslopes(DBonTable,GISdb[DBonTable].MyData.GetFieldByNameAsString(TileNameGDEMorHRDEM(DBonTable)));
end;
...
procedure Tdbtablef.GraphwheretilebestrepresentsDSMorDTM1Click(Sender: TObject);
begin
   WhereIsItDSMorDTMorTie(dbOnTable);
end;
...
begin
   with GISdb[DBonTable] do begin
     ...
     if (Sender = AllProfiles1) or (Sender = Allnormalizedprofiles1) then begin
        ...
        if (Sender = AllProfiles1) then Result.RedrawDiagram;

        if (Sender = Allnormalizedprofiles1) then begin
            Result.OpenXYZFile(rfile);
            Result.GraphDraw.MinZ := 0;
            Result.GraphDraw.MaxZ := 0;
            rc := GISdb[DBonTable].MyData.RecordCount;
            ...
            for i := 0 to 100 do begin
               ...
               for j := 0 to 100 do
                  if (PointDensity[i,j] > 0) then begin
                     ...
                     if v3[3] > Result.GraphDraw.MaxZ then Result.GraphDraw.MaxZ := v3[3];
...
begin
   SortDataBase(DBOnTable,false,true);
end;

procedure Tdbtablef.Descending2Click(Sender: TObject);
begin
   SortDataBase(DBOnTable,false,true,SelectedColumn);
end;
...
procedure Tdbtablef.IdentifyDSMandDTM1Click(Sender: TObject);
begin
   {$IfDef RecordDEMIX} WriteLineToDebugFile('IdentifyDSMandDTM1Click in'); {$EndIf}
   FormWorking := true;
   SetColorForProcessing;
   ShowHourglassCursor;
   IdentifyDSMorDTM(dbOnTable);
   SetColorForWaiting;
   FormWorking := false;
   ShowStatus;
...
procedure Tdbtablef.Interpolationresultscriteriaplusfields1Click(Sender: TObject);
var
...
begin
   CriteriaInFUVdb(dbOnTable);
end;

procedure Tdbtablef.IQR1Click(Sender: TObject);
var
   Values : ^bfarray32;
   NPts : int64;
begin
   New(Values);
   GISDB[DBonTable].GetPointArrayForDBField(SelectedColumn,Values^,NPts);
   MessageToContinue('IQR for field ' + SelectedColumn + ':' + RealToString(Petmath.CalculateIQR(NPts,Values^),-12,-2));
   Dispose(Values);
end;
...
procedure Tdbtablef.DSMDTMpowerlawforalltiles2Click(Sender: TObject);
begin
...
procedure Tdbtablef.BestreferenceDEMforALOS1Click(Sender: TObject);
begin
   FindBestReferenceDEM(dbOnTable,'ALOS');
end;
...
function DEMIX_DTMtestrefdir(db : integer) : PathStr;
begin
   Result := ExtractFilePath(GISdb[DB].DBFullName) + GISdb[DB].MyData.GetFieldByNameAsString('AREA')  + '\' + GISdb[DB].MyData.GetFieldByNameAsString('DTM_NAME')  + RefTestStr + '\';
end;

function DEMIX_DSMtestrefdir(db : integer) : PathStr;
begin
   Result := ExtractFilePath(GISdb[DB].DBFullName) + GISdb[DB].MyData.GetFieldByNameAsString('AREA')  + '\' + GISdb[DB].MyData.GetFieldByNameAsString('DSM_NAME')  + RefTestStr + '\';
end;
...
procedure Tdbtablef.LoadtestandreferenceDEMs2Click(Sender: TObject);
            ...
            var
               ...
               fName : PathStr;
            begin
               if ValidPath(theDir) then begin
                  TheDEMs := Nil;
...
begin
   Process(DEMIX_DTMtestrefdir(dbOnTable), '');
   Process(DEMIX_DSMtestrefdir(dbOnTable), 'ref_DSM');
end;
...
procedure Tdbtablef.Capitalize1Click(Sender: TObject);
var
   aString : ShortString;
begin
    ShowHourglassCursor;
    ...
    GISdb[DBonTable].MyData.First;
    while not GISdb[DBonTable].MyData.eof do begin
       aString := UpperCase(GISdb[DBonTable].MyData.GetFieldByNameAsString(SelectedColumn));
       GISdb[DBonTable].MyData.Edit;
       GISdb[DBonTable].MyData.SetFieldByNameAsString(SelectedColumn,aString);
       GISdb[DBonTable].MyData.Next;
    end;
...
procedure Tdbtablef.Cret1Click(Sender: TObject);
var
   Stats,TheFields : tStringList;
   MomentVar : tMomentVar;
   fName : PathStr;
   db,i : integer;
begin
   ...
   Stats.Add(MomentStr);
   TheFields := GISdb[DBonTable].GetMultipleNumericFields('Box plots');
   for i := 0 to pred(TheFields.Count) do begin
      MomentVar := GISdb[DBonTable].GetFieldStatistics(TheFields.Strings[i]);
      Stats.Add(TheFields.Strings[i] + MomentResultsToString(MomentVar));
   end;
    fName := NextFileNumber(MDTempDir,'box_plot','.dbf');
...
begin
   if ((Sender = FillField1) or (Sender = ClearField1)) and (SelectedColumn <> '') then WantedFieldName := SelectedColumn
...
begin
   ...
   if CheckBox1.Enabled and ValidDB(DBonTable) then begin
...
begin
   ...
   while not GISdb[DBonTable].MyData.eof do begin
      ...
      GISdb[DBonTable].aShapeFile.OldGetLineCoordsWithZs(GISdb[DBonTable].MyData.RecNo,NumPts,NParts,Coords^,PartSize,ShapeFile3D(GISdb[DBonTable].ShapeFileType),zs^);
      for i := 0 to pred(NumPts) do begin
         DEMGlb[NewDEM].SetGridElevLatLongDegree(Coords^[i].Lat,Coords^[i].Long,zs^[i]);
         if DEMGlb[GISdb[DBonTable].TheMapOwner.MapDraw.DEMonMap].GetElevFromLatLongDegree(Coords^[i].Lat,Coords^[i].Long,z) then begin
            DEMGlb[DiffDEM].SetGridElevLatLongDegree(Coords^[i].Lat,Coords^[i].Long,zs^[i]-z);
         end;
      end;
      Dispose(Coords);
      Dispose(zs);
   ...
   if (HowCreate = cgPointDensity) then begin
      ...
      while not GISdb[DBonTable].MyData.eof do begin
         ...
         if GISdb[DBonTable].ValidLatLongFromTable(Lat,Long) then begin
            ...
            if DEMGlb[Result].GridInDataSetInteger(xg,yg) then begin
...
function WhichReferenceIsItCloserTo(TestDEM,RefDTM,RefDSM: integer; Tolerance : float32; AName : shortString) : integer;
var
   i,What : integer;
   RefDSMz,TestZ,RefDTMz : float32;
   x,y : integer;
   VAT : tStringList;
...
begin
   ...
   Result := DEMGlb[TestDEM].CloneAndOpenGridSetMissing(ByteDEM,AName,euIntCode);
   for x := 0 to pred(DEMGlb[TestDEM].DEMheader.NumCol) do begin
      for y := 0 to pred(DEMGlb[TestDEM].DEMheader.NumRow) do begin
         if DEMGlb[TestDEM].GetElevMetersOnGrid(x,y,TestZ) and DEMGlb[RefDTM].GetElevMetersOnGrid(x,y,RefDTMz) and DEMGlb[RefDSM].GetElevMetersOnGrid(x,y,RefDSMz) then begin
               RefDSMz := abs(RefDSMz - TestZ);
               RefDTMz := abs(RefDTMz - TestZ);
               //if (RefDSMz <= Tolerance) and (RefDTMz <= Tolerance) then What := 2
               if abs(RefDSMz - RefDTMz) <= Tolerance then What := 2
               else if RefDTMz > RefDSMz then What := 3
               else What := 1;
   ...
   aName := MDtempDir + aName + '.tif';
   DEMglb[Result].SaveAsGeotiff(AName);
   VAT := tStringList.Create;
   Vat.add('VALUE,NAME,N,USE,COLOR');

   if (Hist[3] > 0) then Vat.add('3,DSM,' + IntToStr(Hist[3]) + ',Y,' + IntToStr(clBlue));
   if (Hist[2] > 0) then Vat.add('2,Tie,' + IntToStr(Hist[2]) + ',Y,' + IntToStr(clYellow));
   if (Hist[1] > 0) then Vat.add('1,DTM,' + IntToStr(Hist[1]) + ',Y,' + IntToStr(clLime));
   aName := ChangeFileExt(aname,'_vat.dbf');
...
procedure Tdbtablef.MapcomparingtestDEMtoDSMandDTM1Click(Sender: TObject);
var
   LC,Cop,CHM,Ref_DTM,REF_DSM,WhichBest : integer;
   fName : PathStr;
begin
   Cop := OpenNewDEM(DEMIX_DTMtestrefdir(dbOnTable) + 'cop.tif');
   LC := OpenNewDEM(DEMIX_DTMtestrefdir(dbOnTable) + 'landcover.tif');
   Ref_DTM := OpenNewDEM(DEMIX_DTMtestrefdir(dbOnTable) + 'ref_dtm_srtm.tif');
   Ref_DSM := OpenNewDEM(DEMIX_DSMtestrefdir(dbOnTable) + 'ref_dsm_srtm.tif');
   WhichBest := WhichReferenceIsItCloserTo(Cop,Ref_DTM,Ref_DSM,1,'COP_closer_to');
   CHM := MakeDifferenceMap(Ref_DSM,Ref_DTM,Ref_DTM,Ref_DTM,true,'CHM');
   fName := MDtempDir + 'chm.tif';
   DEMglb[CHM].SaveAsGeotiff(fName);
end;
...
procedure Tdbtablef.PowerlawforHRDEMDSMDTM1Click(Sender: TObject);
begin
   ComputePowerFitForAllTiles(dbOnTable);
end;
   ...
   with GISdb[DBonTable] do begin
      ...
      while not GISdb[DBonTable].MyData.EOF do begin
         ...
         if ValidLatLongFromTable(Lat,Long) then begin
           ...
           if (TStr <> '') then begin
              z := StrToFloat(TStr);
              DEMGlb[TheMapOwner.MapDraw.DEMonMap].SetGridElevLatLongDegree(Lat,Long,z);
           end;
         end;
         GISdb[DBonTable].MyData.Next;
      end;
...
begin
   //with GISdb[DBonTable] do begin
     ...
     GISdb[DBonTable].MyData.First;
     while not GISdb[DBonTable].MyData.EOF do begin
        if GetKoppenColor(GISdb[DBonTable].MyData.GetFieldByNameAsString('CLASS'),Color) then begin
           GISdb[DBonTable].MyData.Edit;
...
begin
   //with GISdb[DBonTable] do begin
       ...
       while not GISdb[DBonTable].MyData.EOF do begin
          ...
          GISdb[DBonTable].aShapeFile.GetLineCoords(GISdb[DBonTable].MyData.RecNo);
          TStr := '';
          if (GISdb[DBonTable].aShapeFile.CurrentPolyLineHeader.NumParts > 1) then begin
             if AreaShapeFile(GISdb[DBonTable].ShapeFileType) then begin
                 GISdb[DBonTable].aShapeFile.AreasCounterClockwise;
                 Outs := 0;
                 Holes := 0;
                // with GISdb[DBonTable].aShapeFile.CurrentPolyLineHeader do begin
                    for j := 1 to GISdb[DBonTable].aShapeFile.CurrentPolyLineHeader.NumParts  do begin
                       if GISdb[DBonTable].aShapeFile.CurrentLinePartsCCW[j] then inc(holes)
                       else inc(Outs);
                    end;
             ...
             if (Sender = Recordswithholes1) then begin
                if Holes > 0 then Results.Add(GISdb[DBonTable].MyData.GetFieldByNameAsString(CheckField) + '  holes=' + IntToStr(Holes));
             end
             else Results.Add(GISdb[DBonTable].MyData.GetFieldByNameAsString(CheckField) + '  Parts=' +
                IntToStr(GISdb[DBonTable].aShapeFile.CurrentPolyLineHeader.NumParts) + '  ' + TStr);
          end;
          GISdb[DBonTable].MyData.Next;
       end;
       ShowStatus;
       Petmar.DisplayAndPurgeStringList(Results,'Multi-part records in ' + GISdb[DBonTable].dbName);
   //end;
end;
...
begin
   ...
   New(zs);
   GISdb[DBonTable].GetFieldValuesInArrayLinkPossible(SelectedColumn,zs^,Npts,Missing,Min,Max);
   Petmath.HeapSort(npts,zs^);
   for i :=  0 to pred(NPts) do begin
      Results.Add(RealToString(100 * i / NPTs,8,1) + ',' + RealToString(zs[i],-18,-6));
   end;
   Dispose(zs);
   fName := MDTempdir + 'Percentiles for ' + SelectedColumn + '.dbf';
...
begin
   FieldPercentileReport(DBonTable);
end;
