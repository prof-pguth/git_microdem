...
begin
   ...
   if FLastResult = S_OK then FRevisionNumber := info.RevisionNumber;
...
------------------------------------------------------------------------------*}
procedure T3DConnexionDevice.FireKeyChange(const KeyIndex: integer; const Pressed: boolean);
...
var cX, cZ: cardinal;
begin
   if Assigned(FOnRotation) then begin
      FOnRotation(X, Y, Z, Angle);
   end;
   case FEmulationType of
      etNone: ;
      etMouse, etMouseKeyboard: begin
...
procedure T3DConnexionDevice.HandleCustomDetail(const Axis: TEmulationAxis; const Value: Double);
